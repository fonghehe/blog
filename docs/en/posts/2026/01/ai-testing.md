...
The biggest cost of testing isn't writing tests — it's maintaining them. AI can monitor test failures, automatically fix tests broken by code changes, and even flag outdated tests.
...
async function autoFixTests() {
  // Run tests, collect failure info
  ...
  for (const failure of failures) {
    // Pass failure info and diff together to AI
    const diff = execSync("git diff HEAD~1", { encoding: "utf-8" });
    const fixPrompt = `
Test file ${failure.name} failed.
Failure message: ${failure.message}
Recent code changes: ${diff}
Please analyze the failure cause and fix the test. If the expected behavior changed due to code changes,
update the test assertions; if it's a regression bug, flag it for human review.
    `;

    // Call AI to fix
    // AI distinguishes between "test needs updating" and "code has a bug"
  }
}
```
...
- AI test generation requires risk context — blindly asking "write tests" produces garbage
- AI already does well with unit tests; E2E tests need more precise prompt guidance
- Coverage numbers matter less than AI's risk analysis — prioritize covering high-risk paths
- Test maintenance costs far exceed authoring costs; AI auto-fixing broken tests is where the real value lies
- Good testing strategy: AI handles breadth (coverage), humans handle depth (correctness of critical paths)
