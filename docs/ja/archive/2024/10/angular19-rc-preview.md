...
tags:
  - Angular
readingTime: 3
description: "Angular 19 RC のプレビューが利用可能になりました。正式版は2024年11月19日にリリース予定です。本記事では RC バージョンをベースに、最も重要な新機能である**インクリメンタルハイドレーション**と**テンプレート HMR**、およびルートレベルレンダリングモードの更なる改善を紹介します。"
wordCount: 774
---

Angular 19 RC のプレビューが利用可能になりました。正式版は2024年11月19日にリリース予定です。本記事では RC バージョンをベースに、最も重要な新機能である**インクリメンタルハイドレーション**と**テンプレート HMR**、およびルートレベルレンダリングモードの更なる改善を紹介します。

> 本記事は Angular 19 RC をベースにしており、APIは正式版と若干異なる場合があります。

## インクリメンタルハイドレーション

Angular 17 は非破壊的 SSR ハイドレーションを導入しましたが、依然として全量ハイドレーション——ページのすべてのコンポーネントが同時に活性化されます。Angular 19 では**インクリメンタルハイドレーション**が導入されました：コンポーネントはオンデマンドで遅延ハイドレーションでき、`@defer` ブロックの遅延ロードロジックに似ています。
...
Angular 19 はコンポーネント**テンプレート**のホットアップデートを正式にサポートします——`.html` テンプレートファイルを修正後、ページ全体をリフレッシュする必要なく、対応するコンポーネントのビューのみが更新され、アプリケーション状態が保持されます：
