...
問題がある場合は具体的な修正コードを提示してください。
  ...
  breakingChange: `
このPRにbreaking changeが導入されているか分析してください：
1. エクスポートされたAPIのシグネチャに変更がないか
2. コンポーネントのpropsの削除や型の変更がないか
3. CSS class名の変更がないか（外部のコンシューマーに影響）
4. 環境変数や設定キーの変更がないか

breaking changeが検出された場合は影響範囲を説明し、移行パスを提案してください。
  `,
...
const reviewWorkflow = {
  aiFirstPass: {
    ...
    // AI は提案を生成するだけで、コードを直接修正しない
  },
  humanReview: {
    showAIInsights: true,
    showConfidenceScore: true,
    // 低信頼度の提案は折りたたみ表示、高信頼度の提案はハイライト表示
