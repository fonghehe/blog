...
テストの最大のコストは書くことではなく、メンテナンスすることだ。AI はテストの失敗を監視し、コード変更によるテストの破損を自動修復し、時代遅れになったテストをマークすることさえできる。
...
async function autoFixTests() {
  ...
  for (const failure of failures) {
    // 失敗情報と diff を一緒に AI に渡す
    const diff = execSync("git diff HEAD~1", { encoding: "utf-8" });
    const fixPrompt = `
テストファイル ${failure.name} が失敗しました。
失敗メッセージ：${failure.message}
直近のコード変更：${diff}
失敗原因を分析し、テストを修正してください。コード変更による期待される動作の変化であれば、
テストのアサーションを更新してください。回帰バグであれば、人間による処理が必要とマークしてください。
    `;

    // AI を呼び出して修復する
    // AI は「テストの更新が必要」と「コードにバグがある」の2つのケースを区別する
  }
}
```
...
- AI テスト生成にはリスク箇所のコンテキストが必要だ。「テストを書いて」とやみくもに頼んでもゴミが出力されるだけ
- 単体テストは AI がすでにうまく処理できる。E2E テストにはより精密な prompt 誘導が必要
- カバレッジ数値は AI のリスク分析ほど意味がない。高リスクパスを優先的にカバーする
- テストメンテナンスコストは作成コストを大きく上回る。AI による破損テストの自動修復こそが真の価値
- 良いテスト戦略：AI が广度（カバレッジ）を担当し、人間が深度（核心パスの正確性）を担当する
