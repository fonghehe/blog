...
tags:
  - ES6
readingTime: 1
description: "ES6 引入了 `Map` 和 `Set` 兩種新的數據結構，但工作中很多人還是習慣用對象和數組。來看看什麼場景下它們更合適。"
wordCount: 166
---

ES6 引入了 `Map` 和 `Set` 兩種新的數據結構，但工作中很多人還是習慣用對象和數組。來看看什麼場景下它們更合適。
...
// 轉換
const obj = Object.fromEntries(map); // Map → 對象
const arr = [...map]; // Map → 數組
```
...
- `Set`：不重複值的集合；最常用：數組去重、集合運算
- `Map`：鍵值對，鍵可以是任意類型；適合：頻繁增刪、鍵非字符串
- 普通場景用對象/數組就夠了，不要為了用新 API 而用