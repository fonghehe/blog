...
tags:
  - 移動端
readingTime: 1
description: "移動端 1px 問題是做手機頁面必須解決的經典問題。根本原因是設備像素比（DPR）。"
---

移動端 1px 問題是做手機頁面必須解決的經典問題。根本原因是設備像素比（DPR）。
...
iPhone 6 的屏幕：
  物理分辨率：750 × 1334 像素
  邏輯分辨率：375 × 667 CSS 像素
  DPR（設備像素比）：2

所以 CSS 裏寫 1px，實際顯示是 2 個物理像素寬
在 Retina 屏幕上看起來比設計稿要"粗"
```
...
```html
<!-- 動態設置 initial-scale，讓 1px CSS 等於 1px 物理像素 -->
<meta name="viewport" content="width=device-width,initial-scale=0.5" />
```

```javascript
// 根據 DPR 動態設置 viewport
...
- 根本原因：Retina 屏 DPR > 1，CSS 1px = 2 個物理像素
