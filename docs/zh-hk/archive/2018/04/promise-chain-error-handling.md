...
tags:
  - JavaScript
readingTime: 1
description: "之前寫了 Promise 基礎，這次專門説鏈式調用和錯誤處理，這塊容易出細節問題。"
wordCount: 160
---

之前寫了 Promise 基礎，這次專門説鏈式調用和錯誤處理，這塊容易出細節問題。
...
fetchUser(1)
  ...
  .catch((err) => {
    console.error("任何一步出錯都會到這裏", err);
  });
```

關鍵：`then` 的回調返回什麼，下一個 `then` 就接到什麼。
...
fetchUser(1)
  ...
  .then(saveUser)
  .catch((err) => {
    // 鏈中任何一個 rejection 都會跳到這裏
    console.error(err);
  });

// 方式二：每個 then 的第二個參數（不推薦，容易漏掉）
fetchUser(1)
...
fetchUser(1)
  ...
  .then((orders) => {
    console.log(orders); // 正確的 orders 數據
  });
```
