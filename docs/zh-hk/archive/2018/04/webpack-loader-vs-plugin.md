...
Webpack 配置裏 `module.rules`（loader）和 `plugins` 是兩個最重要的配置項，但很多人搞不清楚它們的區別。
...
-------- | ---------------- | ------------------------------- |
| 作用範圍 | 單個文件         | 整個構建流程                    |
| 觸發時機 | 文件被 import 時 | Webpack 生命週期鈎子            |
| 配置位置 | `module.rules`   | `plugins` 數組                  |
| 典型用途 | 文件類型轉換     | 壓縮、拆包、生成 HTML、注入變量 |