...
tags:
  - Vue
readingTime: 1
description: "Vue 的響應式系統對對象屬性和數組有一些特殊限制，搞清楚這些才能正確用 `watch`。"
---

Vue 的響應式系統對對象屬性和數組有一些特殊限制，搞清楚這些才能正確用 `watch`。
...
**deep 的代價**：會遍歷對象所有屬性，性能開銷大。如果只關心某個屬性，直接監聽那個路徑：
...
- Vue 2 數組限制：索引賦值和 length 變化檢測不到，用 `$set` 或數組方法
