...
CLI 2 需要手動維護 webpack.config.js，文件又長又難懂。CLI 3 把 webpack 配置完全封裝起來，開箱即用：
...
在代碼裏用 `process.env.VUE_APP_*` 訪問（只有 `VUE_APP_` 前綴的才會暴露給客户端）。
