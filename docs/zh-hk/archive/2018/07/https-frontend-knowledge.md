---
title: "HTTPS 工作原理和前端相關知識：落地路徑與實戰建議"
...
```
1. 客户端 → 服務器：我支持的 TLS 版本、加密算法列表
2. 服務器 → 客户端：選定的加密算法 + 數字證書（含公鑰）
3. 客户端：驗證證書合法性（CA 簽名）
4. 客户端 → 服務器：用公鑰加密一個隨機數
5. 雙方：用隨機數派生出對稱加密密鑰
6. 之後通信：用對稱密鑰加密（速度快）
```
...
```bash
# 用 mkcert 創建本地可信證書（不需要 --ignore-certificate-errors）
mkcert -install
...
```
服務器告訴瀏覽器：以後都用 HTTPS 訪問我，不要用 HTTP

響應頭：
Strict-Transport-Security: max-age=31536000; includeSubDomains
```

瀏覽器會記住這個指令，即使用户輸入 `http://example.com`，也會自動改成 HTTPS。
...
OV（組織驗證）：驗證域名 + 組織信息，需要幾天
  - 顯示公司名，適合企業網站

EV（擴展驗證）：最嚴格，需要 1-2 周
  - 瀏覽器地址欄顯示綠色公司名（現在大多數瀏覽器取消了綠色）
  - 銀行、大型電商用
```
