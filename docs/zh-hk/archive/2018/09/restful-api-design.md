...
✅ 好的設計（名詞複數，層級表示從屬關係）：
GET    /users                   獲取用户列表
GET    /users/123               獲取用户 123
POST   /users                   創建用户
PUT    /users/123               更新用户 123（全量）
PATCH  /users/123               更新用户 123（部分）
DELETE /users/123               刪除用户 123
GET    /users/123/orders        用户 123 的訂單列表
GET    /users/123/orders/456    用户 123 的訂單 456
...
4xx 客户端錯誤：
  ...
  401 Unauthorized       未登錄（需要認證）
  403 Forbidden          已登錄但無權限
  404 Not Found          資源不存在
  409 Conflict           衝突（如用户名已存在）
  422 Unprocessable      參數驗證失敗
...
GET /users?fields=id,name,email            字段選擇
