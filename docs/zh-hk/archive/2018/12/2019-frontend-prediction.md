...
微信小程序、支付寶小程序、字節跳動小程序都在發展。跨平臺小程序框架（uni-app、Taro）會繼續改進。前端要會寫小程序越來越重要。
...
- **Node.js**：學會用 Node 解決工程化問題，不隻是寫 API
