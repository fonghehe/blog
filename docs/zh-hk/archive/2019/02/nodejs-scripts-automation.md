---
title: "Node.js 腳本開發實踐：自動化你的工作流"
date: 2019-02-19 14:43:49
tags:
  - TypeScript
readingTime: 2
description: "前端工程師寫 Node.js 腳本，能解決很多重複勞動。整理一些實用的腳本開發技巧。"
wordCount: 123
---
...
- `commander` 解析命令行參數，`inquirer` 做交互式問答
- `fs-extra` 是 `fs` 的增強版，支持 Promise 和額外方法（ensureDir、copy、move）
- 腳本放 `scripts/` 目錄，通過 `npm scripts` 暴露給團隊
- 用 `#!/usr/bin/env node` 讓腳本直接執行
