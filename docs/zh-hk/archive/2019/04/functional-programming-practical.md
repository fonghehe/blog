...
tags:
  - 前端
readingTime: 1
description: "函數式編程在前端圈一直有熱度，Redux、RxJS 都有它的影子。整理一些實際能用上的概念。"
---

函數式編程在前端圈一直有熱度，Redux、RxJS 都有它的影子。整理一些實際能用上的概念。

## 純函數

```javascript
// 純函數：相同輸入始終相同輸出，無副作用
const add = (a, b) => a + b; // ✅ 純函數
...
const nextState = produce(state, (draft) => {
  draft.count += 1;
  draft.user.age = 26; // 像直接修改，但返回新對象
});
```
...
- 純函數：無副作用，相同輸入相同輸出，便於測試和緩存
- 柯里化：固定部分參數，創建特化函數
- compose/pipe：組合小函數成大函數
- 不可變數據：不修改原數據，Immer 讓寫法更自然
- 不必"全函數式"，在合適的地方用這些概念就夠了
