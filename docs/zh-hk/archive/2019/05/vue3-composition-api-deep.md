---
title: "Vue 3 Composition API 深度解析：RFC 階段的技术預覽"
...
Options API 在簡單組件中很好用，但複雜組件有明顯痛點：
...
state.value.user = { name: 'test' }; // 訪問 .value 後就是普通對象
```
...
- 邏輯複用從 mixins 變成 composables（函數），更清晰、無命名衝突
- `ref` 和 `reactive` 選擇看場景，建議統一用 `ref` 降低心智負擔
- Options API 在 Vue 3 中仍然支持，簡單組件用 Options API 也沒問題
