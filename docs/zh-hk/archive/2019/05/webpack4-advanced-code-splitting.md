---
title: "Webpack 4 Tree Shaking 原理深度解析：落地路徑與實戰建議"
...
這種寫法更精細：隻有列表中的檔案有副作用（比如 CSS 檔案通過 `import './style.css'` 引入，沒有導出但不能刪），其他模塊可以安全地 Tree Shaking。
...
Webpack 4 默認使用 UglifyJS 作為壓縮工具，但社區正在遷移到 **Terser**，因為 UglifyJS 不支援 ES6+ 語法。
...
還有一個小技巧：在 `package.json` 中不設定 `sideEffects` 時，Webpack 會輸出警告提示哪些模塊可能需要設定。
