---
title: "前端 Code Review 要點：實踐方法與治理思路"
...
風格不統一的代碼讀起來很痛苦。在 2019 年，ESLint + Prettier 的組合已經是標配了。
...
// 函數名動詞缺失
function userInfo(id) { /* 是獲取還是設置？ */ }
function data(list) { /* 做了什麼？ */ }

// 布爾值命名不直觀
const status = checkLogin()  // status 是 true/false 還是狀態字符串？
```
...
一些經驗：
- 變量名用名詞，函數名用動詞開頭
- 布爾值用 `is`/`has`/`can`/`should` 前綴
- 避免縮寫，`btn` 不如 `button`，`cb` 不如 `callback`
- 數組用複數形式，`users` 比 `userList` 更自然
- 事件處理函數用 `handle` 或 `on` 前綴，保持一致

**命名一致性**：同一個概念在項目中應該用同一個詞。比如"用户信息"要麼全叫 `userInfo`，要麼全叫 `userData`，不要混着用。

## 效能考量

CR 時需要留意常見的效能問題，尤其是渲染相關的。

**1. 避免在 render 中創建新對象/函數**

```jsx
{% raw %}
// 反面：每次 render 都創建新的 style 對象和新的回調函數
...
```jsx
// 反面：用 index 作為 key（數據變化時會導致不必要的重渲染）
...
```html
<!-- 2019 年原生懶加載還不成熟，需要用 IntersectionObserver -->
...
```jsx
{% raw %}
// 反面：直接插入用户輸入的 HTML，XSS 風險極大
function Comment({ content }) {
  return <div dangerouslySetInnerHTML={{ __html: content }} />
}

// 正面：React 默認就會轉義，直接渲染字符串即可
function Comment({ content }) {
...
無障礙在國內項目中容易被忽視，但 CR 時應該養成檢查的習慣。

```html
<!-- 反面：div 模擬按鈕，屏幕閲讀器無法識別 -->
...
簡單的檢查清單：
- 所有可交互元素能否用鍵盤操作（Tab 切換、Enter/Space 觸發）
...
CR 時看組件設計，重點關注**職責單一**和**可複用性**。

**反面案例：一個組件做太多事**

```jsx
// UserProfile.js：既獲取數據，又處理業務邏輯，又渲染 UI
...
// components/UserProfile.js —— 純 UI 組件
...
function UserProfileContainer({ userId }) {
  ...
  return (
    <div>
      ...
      <button onClick={handleChat}>發消息</button>
    </div>
  )
}
```

拆分後的好處：`UserProfile` 可以在任何地方複用，不需要關心數據從哪來；`useUser` hook 也可以被其他組件複用。

## 錯誤處理

這是 CR 中最常被遺漏的部分。沒有錯誤處理的代碼是定時炸彈。

**反面案例：網絡請求沒有錯誤處理**

```javascript
// fetch 不 catch 錯誤，接口掛了頁面就白屏
...
- 代碼風格交給 ESLint + Prettier 自動化，CR 不應該花時間在格式問題上
- 關注命名：變量名是名詞，函數名是動詞開頭，布爾值用 is/has/can/should
- 性能重點看渲染相關：避免 render 中創建新引用、列表 key 要穩定
- 安全性第一防線是 XSS，堅決杜絕 dangerouslySetInnerHTML / v-html 插入未經處理的用户輸入
- 組件設計遵循單一職責，數據獲取、業務邏輯、UI 渲染應該分離
- 錯誤處理不是可選項，每個網絡請求和異步操作都必須有兜底方案
