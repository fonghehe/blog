...
白名單模式，只發布 `files` 中列出的文件。簡潔明確，不容易出錯。注意 `package.json`、`README.md`、`LICENSE` 和 `CHANGELOG` 始終會被包含，不需要顯式列出。
...
推薦用 `files` 字段，原因很簡單：白名單更安全，且不需要維護額外的文件。
...
- `npm unpublish`：從 registry 徹底刪除，其他依賴它的項目會安裝失敗。npm 有 72 小時限制，且如果有人依賴了你的包則不允許 unpublish
- `npm deprecate`：包還在，只是安裝時會顯示警告。對已有用户更友好
...
- 用 `files` 字段白名單控制發佈內容，比 `.npmignore` 更安全
