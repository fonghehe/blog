---
title: "Electron 桌面應用開發入門：落地路徑與實戰建議"
date: 2019-07-11 16:34:20
tags:
  - 跨平臺
readingTime: 5
description: "最近團隊需要做一個內部工具——JSON 格式化/轉換桌面小工具。Web 版本當然可以做，但涉及到本地檔案讀寫、系統剪貼板、快捷鍵註冊等能力，最終選擇了 Electron。這篇文章記錄了從零搭建到打包分發的完整過程。"
wordCount: 658
---

最近團隊需要做一個內部工具——JSON 格式化/轉換桌面小工具。Web 版本當然可以做，但涉及到本地檔案讀寫、系統剪貼板、快捷鍵註冊等能力，最終選擇了 Electron。這篇文章記錄了從零搭建到打包分發的完整過程。
...
確保 `electron-builder.json` 的 `files` 字段包含了所有需要的檔案。如果用了 Vue CLI 或 Webpack，需要指定打包輸出目錄：
...
- `electron-updater` 實現自動更新，需要搭配靜態檔案服務器或 GitHub Releases
