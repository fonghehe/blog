---
title: "前端安全之 CSP (Content Security Policy)：實踐方法與治理思路"
date: 2019-07-04 14:30:53
tags:
  - 安全
readingTime: 4
description: "前端安全是每個開發者都應該重視的話題。XSS 攻擊防不勝防，單純靠過濾用户輸入很難做到萬無一失。CSP（Content Security Policy）提供了一種從瀏覽器層面限製資源加載和腳本執行的機製，是 XSS 防禦的重要補充。"
wordCount: 865
---

前端安全是每個開發者都應該重視的話題。XSS 攻擊防不勝防，單純靠過濾用户輸入很難做到萬無一失。CSP（Content Security Policy）提供了一種從瀏覽器層面限製資源加載和腳本執行的機製，是 XSS 防禦的重要補充。
...
這行設定意味着：所有資源（腳本、樣式、圖片、字體等）隻能從同源加載。
...
| `base-uri`        | `<base>` 標籤                              |
| `form-action`     | 表單提交目標                               |
| `frame-ancestors` | 誰可以嵌套當前頁面（替代 X-Frame-Options） |
...
Webpack 打包後默認會生成 inline runtime 代碼。要配合 CSP，需要做如下設定：
...
還有 `Content-Security-Policy-Report-Only` 頭，隻上報不阻止，適合上線前的灰度測試：
...
- `style-src 'unsafe-inline'` 很多項目暫時無法避免，因為 Vue/React 的組件庫大量使用 inline style
...
- nonce 方案比 `'unsafe-inline'` 安全得多，建議腳本全部使用 nonce
- 配置 `report-uri` 監控線上 CSP 違規，先用 Report-Only 模式收集數據再切換到 enforce
