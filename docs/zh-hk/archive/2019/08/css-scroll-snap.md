---
title: "CSS Scroll Snap 滾動吸附效果：落地路徑與實戰建議"
date: 2019-08-19 10:44:46
tags:
  - CSS
readingTime: 4
description: "圖片輪播、全屏滾動頁面、橫向滾動卡片——這些常見的交互效果過去往往需要依賴 JavaScript 庫實現。CSS Scroll Snap 屬性讓我們可以用純 CSS 實現滾動吸附效果，效能更好、代碼更簡潔。2019 年，Scroll Snap 已經獲得了主流瀏覽器的廣泛支援。本文將系統講解 Scroll Snap 的用"
wordCount: 642
---

圖片輪播、全屏滾動頁面、橫向滾動卡片——這些常見的交互效果過去往往需要依賴 JavaScript 庫實現。CSS Scroll Snap 屬性讓我們可以用純 CSS 實現滾動吸附效果，效能更好、代碼更簡潔。2019 年，Scroll Snap 已經獲得了主流瀏覽器的廣泛支援。本文將系統講解 Scroll Snap 的用法和實戰案例。

## 核心概念

Scroll Snap 涉及兩個關鍵屬性：

- **`scroll-snap-type`** — 設置在滾動容器上，定義滾動軸和吸附嚴格程度
- **`scroll-snap-align`** — 設置在子元素上，定義吸附位置
...
scroll-snap-type: x proximity;   /* 寬鬆吸附，儘量對齊但不強製 */
```

- **`mandatory`** — 滾動停止時必須對齊到吸附點。適用於輪播圖、全屏滾動等精確場景
- **`proximity`** — 滾動停止時儘量對齊，但位置不夠精確時不會強製調整。適用於長列表等自然滾動場景
...
<div class="fullpage-scroll">
  ...
  <section class="section" id="contact">聯繫</section>
</div>
```
...
## scroll-snap-stop 控製滾動穿透

默認情況下，快速滑動可能跳過多個吸附點。`scroll-snap-stop` 可以控製這個行為：
...
這個屬性特別適合需要用户逐頁閲讀的場景，如新手引導頁。

## scroll-padding 和 scroll-margin

### scroll-padding（設置在容器上）

在滾動容器內設置內邊距，不影響佈局但影響吸附位置：
...
/* 不支援的瀏覽器仍然可以正常使用滾動，隻是沒有吸附效果 */
```

## 小結

- `scroll-snap-type` 設置在容器上定義滾動軸和吸附嚴格度，`scroll-snap-align` 設置在子元素上定義吸附位置
...
- 使用 `@supports` 做漸進增強，不支持的瀏覽器仍可正常使用滾動功能
- 純 CSS 實現，性能優於 JavaScript 方案，推薦在新項目中優先使用
