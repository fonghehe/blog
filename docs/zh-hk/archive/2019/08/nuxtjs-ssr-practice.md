...
export default {
  ...
  // 構建優化
  build: {
    extractCSS: true, // CSS 單獨提取（更好的緩存）
    optimizeCSS: true,
    babel: {
...
2. **第三方庫不支持 SSR**：用 `mode: 'client'` 插件或動態導入
3. **cookies**：服務端請求要轉發 cookie，用 `@nuxtjs/proxy` 解決跨域

## 小結

- `asyncData` 在服務端執行，返回的數據合併到 data，直接用於渲染
- 動態路由 `_id.vue` 對應 `:id` 參數
- `head()` 方法設置 SEO meta 標籤
- 靜態生成適合內容不常變的頁面，SSR 適合實時數據
