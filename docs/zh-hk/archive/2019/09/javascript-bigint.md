...
tags:
  - JavaScript
readingTime: 3
description: "JavaScript 的 `Number` 類型基於 IEEE 754 雙精度浮點數，安全整數範圍為 `-(2^53 - 1)` 到 `2^53 - 1`（即 `Number.MAX_SAFE_INTEGER` 和 `Number.MIN_SAFE_INTEGER`）。超過這個範圍的整數運算就會出現精度丟失。ES202"
wordCount: 396
---

JavaScript 的 `Number` 類型基於 IEEE 754 雙精度浮點數，安全整數範圍為 `-(2^53 - 1)` 到 `2^53 - 1`（即 `Number.MAX_SAFE_INTEGER` 和 `Number.MIN_SAFE_INTEGER`）。超過這個範圍的整數運算就會出現精度丟失。ES2020 引入的 `BigInt` 類型讓我們可以表示任意精度的整數。本文將深入講解 BigInt 的使用場景、語法和注意事項。
...
const b = BigInt('9007199254740993');           // 構造函數方式

// 從數字創建
const c = BigInt(9007199254740993);             // 不推薦，可能丟失精度
const d = BigInt('9007199254740993');           // 推薦，從字符串創建
...
BigInt 和 Number 不能直接混合運算，必須先轉換類型：
...
對於不支持的環境，可以使用 `big-integer` 或 `bignumber.js` 等 polyfill：
