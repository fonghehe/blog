---
title: "Svelte 響應式基礎語法詳解：落地路徑與實戰建議"
date: 2019-09-02 14:58:08
tags:
  - React
readingTime: 4
description: "Svelte 是一個全新的前端編譯框架，與 React 和 Vue 不同，它沒有虛擬 DOM，在編譯階段就將組件轉換為高效的命令式代碼。Svelte 的響應式系統基於編譯時的賦值追蹤，語法極其簡潔。2019 年 Svelte 3 正式發佈，本文將帶你深入瞭解其響應式原理和核心語法。"
wordCount: 718
---

Svelte 是一個全新的前端編譯框架，與 React 和 Vue 不同，它沒有虛擬 DOM，在編譯階段就將組件轉換為高效的命令式代碼。Svelte 的響應式系統基於編譯時的賦值追蹤，語法極其簡潔。2019 年 Svelte 3 正式發佈，本文將帶你深入瞭解其響應式原理和核心語法。

## Svelte 的核心理念

React 和 Vue 在運行時維護一套虛擬 DOM，通過 diff 算法計算最小更新。Svelte 的思路完全不同：在編譯時分析組件的依賴關係，生成直接操作真實 DOM 的代碼。
...
function create_fragment(ctx) {
  ...
  return {
    c() {
      ...
      button.textContent = `點擊了 ${ctx.count} 次`;
    },
    m(target, anchor) {
    ...
    p(ctx, dirty) {
      if (dirty & 1) {
        set_data(button, `點擊了 ${ctx.count} 次`);
      }
    },
    d(detaching) {
...
Svelte 在編譯時就知道 `count` 變化會影響按鈕的文本，因此生成了精準的 DOM 更新代碼。

## 響應式聲明 $

使用 `$:` 標記的語句是響應式聲明，當依賴的變量變化時自動重新執行：
...
<script>
  ...
  // 響應式聲明：當 width 或 height 變化時重新計算
  $: area = width * height;
...
Svelte 的響應式基於賦值操作，這意味着對數組或對象的修改不會自動觸發更新：
...
Svelte 提供了 writable store 和 readable store 來管理全局狀態：
...
|
------|--------|-------|-----|
| 編譯/運行時 | 純編譯 | 運行時 + 虛擬 DOM | 運行時 + 虛擬 DOM |
| 響應式 | 編譯時賦值追蹤 | 手動 setState/hooks | Object.defineProperty |
| 包體積 | 極小（無框架運行時） | 較大 | 中等 |
| 語法 | `.svelte` 單文件 | JSX | `.vue` 單文件 |
| 學習曲線 | 低 | 中 | 中 |
| 生態 | 較小 | 最大 | 大 |
| 適用場景 | 小型應用、嵌入式組件 | 大型 SPA | 大型 SPA |

## 小結

- Svelte 的響應式基於編譯時賦值追蹤，普通變量賦值即觸發視圖更新
- `$:` 標記響應式聲明，自動追蹤依賴的變量
- 數組和對象的修改需要重新賦值才能觸發響應式更新，或者使用 `obj = obj` 技巧
- `writable` 和 `derived` store 提供了跨組件狀態管理方案
- `$store` 語法糖自動處理訂閲和取消訂閲
- `bind:` 提供了簡潔的雙向綁定語法
- Svelte 適合追求極致性能和最小包體積的場景，但生態不如 React 和 Vue 成熟
