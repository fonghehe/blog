---
title: "CSS Houdini Paint API 探索：落地路徑與實戰建議"
...
截至目前，Chrome 65+ 已經完整支援 Paint API，Edge 基於 Chromium 也支援，Firefox 和 Safari 暫未默認開啓。
...
Paint API 最強大的地方在於它可以讀取 CSS 自定義屬性（CSS Variables），這意味着繪製行為可以完全由 CSS 控製：
...
- `inputProperties` 可以讀取 CSS 自定義屬性，實現聲明式控製
