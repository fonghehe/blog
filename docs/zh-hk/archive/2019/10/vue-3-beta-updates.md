---
title: "Vue 3 Beta 更新內容跟蹤：落地路徑與實戰建議"
...
Vue 2 的代碼庫經過幾年發展，暴露了一些架構上的限製：

1. **代碼組織**：Options API 使得相關邏輯分散在不同選項中，大型組件難以維護
2. **類型推導**：Options API 的對象寫法對 TypeScript 支持有限
3. **性能瓶頸**：虛擬 DOM 的 diff 算法是全量比較，無法跳過靜態內容
4. **體積**：不支持 Tree Shaking，打包整個運行時
...
1. **靜態提升（Static Hoisting）**：靜態節點隻創建一次
2. **Patch Flags**：編譯器為動態節點打標記，diff 隻比較標記的部分
3. **Block Tree**：將包含動態節點的片段提升為 Block
4. **緩存事件處理器**：內聯事件處理器會被緩存
