...
2. **不會增長的 CSS**：只有使用的工具類會被生成
3. **響應式簡單**：前綴語法 `md:`、`lg:` 非常直觀
4. **一致性**：設計系統內置，間距、顏色、字號等都有規範
...
3. **不適合複雜組件**：高度定製化的組件還是需要自定義 CSS
4. **團隊共識**：需要團隊接受原子化 CSS 的理念

## 構建優化

```js
// tailwind.config.js - 生產環境優化
module.exports = {
  purge: [
    './src/**/*.{js,jsx,ts,tsx}',
    './public/index.html',
...
啓用 `purge` 後，Tailwind 會在生產構建時移除未使用的類，CSS 文件大小可以從 3MB+ 降到 10KB 以下。
