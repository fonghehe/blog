...
Optional Chaining 可能是 ES2020 中最常用的特性。它允許在訪問深層嵌套對象屬性時，如果中間某個屬性為 `null` 或 `undefined`，直接返回 `undefined` 而不是報錯。
...
Nullish Coalescing 運算符 `??` 與 `||` 類似，但隻在左側為 `null` 或 `undefined` 時才返回右側值。這個區別在處理 `0`、`''`、`false` 等 falsy 值時非常重要：
...
BigInt 是 JavaScript 新增的基本數據類型，用於表示任意精度的整數。它解決了 `Number.MAX_SAFE_INTEGER` (2^53 - 1) 的限製：
...
try {
  ...
  console.log(error) // 隻知道有一個失敗了，不知道是哪個
}
