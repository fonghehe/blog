...
my-extension/
  ...
  ├── src/
  │   └── extension.ts  # 插件入口
  ├── package.json      # 插件清單（核心配置文件）
  ├── tsconfig.json
  └── .vscodeignore     # 打包時忽略的文件
```
...
插件的入口文件 `extension.ts` 負責註冊命令和初始化：
...
TreeView 是 VS Code 側邊欄展示層級數據的標準方式。對於我們的 TODO 列表，TreeView 非常合適：
...
當需要展示更復雜的內容（圖表、表單、交互式界面）時，TreeView 就不夠用了，需要用 Webview：
...
# 發佈到 Marketplace（需要 Personal Access Token）
...
- `package.json` 的 `contributes` 字段是插件能力聲明的核心，理解它是開發插件的第一步
- 激活事件要精確指定，避免使用 `*` 全量激活影響 VS Code 啓動速度
- 命令註冊是插件的核心邏輯入口，所有 UI 交互都圍繞命令展開
- TreeView 適合展示層級數據，Webview 適合複雜交互界面
- CSP 策略是 Webview 安全的基本要求，務必配置
- 調試體驗很好，F5 即可啓動開發窗口，支持斷點調試
