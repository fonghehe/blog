...
2020 年了，ES2020 的特性已經進入 Stage 4，主流瀏覽器和 Node 14 都支持了。趁着年初，把項目裏能用的新特性梳理一遍，順便統一團隊的編碼風格。

## Optional Chaining (?.)

之前寫過好幾篇關於防禦性編程的文章，現在終於有了語言層面的支持。
...
obj?.method();      // OK：method 不存在時不調用

// 但這種情況要小心
obj.method?.(arg);  // 如果 method 存在但不是函數，?.(arg) 返回 undefined
                    // 而不會報錯，可能掩蓋 bug
```

## Nullish Coalescing (??)

以前用 `||` 做默認值，但 `0`、`''`、`false` 都會被吃掉。
...
const title = config.title ?? '默認標題';       // '' 是合法值
...
這個特性我們團隊盼了很久。之前做批量請求，`Promise.all` 一個失敗全部失敗太坑了。
...
async function loadChartModule() {
  try {
    ...
    console.error('圖表模塊加載失敗，使用基礎版本', err);
...
2. ESLint 規則更新
   - 開啓 optional-chaining 和 nullish-coalescing 的 lint 規則
   - 禁止使用 || 對可能為 0 的數值做默認值
