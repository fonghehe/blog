...
tags:
  - Vue
readingTime: 2
description: "Vue 3 的 Composition API 是自 Vue 誕生以來最重大的範式轉變。對於習慣了 Options API 的開發者來説，這套 API 的學習曲線並不陡峭，但要真正用好它，需要理解其設計哲學和底層機製。本文從實際工程角度出發，剖析 Composition API 的核心用法。"
wordCount: 448
---

Vue 3 的 Composition API 是自 Vue 誕生以來最重大的範式轉變。對於習慣了 Options API 的開發者來説，這套 API 的學習曲線並不陡峭，但要真正用好它，需要理解其設計哲學和底層機製。本文從實際工程角度出發，剖析 Composition API 的核心用法。
...
注意 `ref` 返回的對象在 JS 中訪問需要 `.value`，在範本中則自動解包。這是很多初學者容易踩的坑。
...
Composition API 中的生命週期鈎子都以 `on` 開頭，且隻在 `setup` 中有效。
...
- 生命週期鈎子全部以 `on` 前綴命名，隻能在 `setup` 中調用
