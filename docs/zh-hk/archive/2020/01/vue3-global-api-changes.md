...
Vue 2 通過構造函數創建實例，Vue 3 通過工廠函數創建應用。
...
關鍵區別：每個 `createApp` 實例擁有獨立的全局註冊空間。一個應用註冊的組件不會污染另一個應用。
...
```javascript
// 兼容層寫法：同時支持 Vue 2 和 Vue 3
const install = (appOrVue) => {
  // Vue 3: app.use() 時傳入 app
  // Vue 2: Vue.use() 時傳入 Vue 構造函數
...
使用 `@vue/compat` 兼容層可以在 Vue 3 中繼續運行 Vue 2 代碼，逐步遷移。
...
- `createApp` 替代 `new Vue`，所有全局 API 掛載到 app 實例上
- 每個 app 實例擁有獨立的全局註冊空間，支持多應用隔離
- 事件總線 `$on/$off`、過濾器、`$set/$delete` 已移除，需要尋找替代方案
- 使用 `@vue/compat` 兼容層可以漸進式遷移，不必一步到位
