...
Vue 3 使用 `Proxy` 攔截對象的讀寫操作，配合 `Reflect` 確保 `this` 指向正確。
...
響應式系統用一個三層 Map 結構存儲依賴關係：`targetMap -> depsMap -> dep`。
...
`WeakMap` 確保當 `target` 被垃圾回收時，對應的依賴信息自動清除，避免內存泄漏。

## ref 與 reactive 的區別

`ref` 本質上是對基本類型的包裝，內部用 `reactive` 包裹一個 `{ value: ... }` 對象。
...
class RefImpl {
  constructor(value) {
    this._value = convert(value) // 對象類型會用 reactive 包裹
    this.__v_isRef = true
  }
...
console.log(doubled.value) // 2，直接返回緩存
count.value = 5
