...
tags:
  - React
readingTime: 2
description: "React Hooks 發佈一年多了，從最初的不適應到現在的離不開，總結一些項目中高頻使用的自定義 Hook 模式。"
wordCount: 160
---

React Hooks 發佈一年多了，從最初的不適應到現在的離不開，總結一些項目中高頻使用的自定義 Hook 模式。
...
- `useRef` 解決閉包陷阱，存儲不需要觸發渲染的值
- `useMemo` 和 `useCallback` 配合 `React.memo` 減少不必要的重渲染
- 實際項目中，數據請求、防抖節流、本地存儲是最高頻的三個 Hook 場景
