...
這意味着你改一個檔案，webpack 需要重新構建受影響的整個 chunk，然後通過 WebSocket 發送更新。
...
瀏覽器看到 `type="module"` 後，會自行解析 `import` 語句併發起請求。Vite 隻需要在服務端轉換被請求的檔案：
...
Vue 和 React 的 HMR 外掛（`@vitejs/plugin-vue`、`@vitejs/plugin-react`）自動處理組件級熱更新，開發者通常不需要手寫上面的代碼。
