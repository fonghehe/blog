...
傳統開發服務器（Webpack dev server）：

- 啓動時打包所有模塊
- 項目大 → 啓動時間長（我們項目現在要 60s）
- 熱更新也要重新打包相關模塊

Vite 的做法：

- 啓動時**不打包**，直接利用瀏覽器原生 ESM
- 瀏覽器請求哪個模塊，才處理哪個模塊
- 啓動時間基本恆定（不管項目多大）

```
瀏覽器請求 /src/main.ts
 → Vite 攔截，實時編譯 TypeScript
 → 返回 ES Module
 → 瀏覽器解析 import，發出新請求
 → Vite 處理新請求...
```
...
cd my-app
npm install
npm run dev  # 500ms 啓動！（Webpack 要 60s）
```
...
Vite HMR：
  文件變化 → 直接發 invalidation 信號 → 瀏覽器重新請求該模塊
...
但這個方向是對的，我很確定。等 1.0 穩定後會在項目中使用。

## 小結

- Vite 利用原生 ESM，開發服務器啓動幾乎瞬間
- 零配置支持 TS、Vue、React、CSS 預處理器
