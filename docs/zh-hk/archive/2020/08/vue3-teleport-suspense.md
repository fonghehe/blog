...
Vue 3 新增了兩個內置組件：Teleport 和 Suspense。一個解決 DOM 結構嵌套問題，一個解決異步組件加載狀態問題。這兩個在實際項目中用得非常頻繁，特別是 Teleport，幾乎是模態框組件的標配了。

## Teleport：把組件渲染到 DOM 樹的任意位置
...
多個 Teleport 可以渲染到同一個目標元素，它們按聲明順序追加：
...
有時候在某些條件下需要禁用 Teleport（比如單元測試時）：
...
Suspense 讓我們可以聲明式地處理異步組件的加載狀態。當子組件（或子組件內的 setup 函數）返回 Promise 時，Suspense 會等待 Promise 完成：
...
一個常見的場景：彈窗內有異步數據加載。Teleport 負責渲染位置，Suspense 負責加載狀態：
...
1. **Suspense 仍然是實驗性特性** —— Vue 3 正式發佈時 Suspense 還是 experimental 狀態，API 可能會變，生產環境慎用
2. **Teleport 的組件實例關係不變** —— 雖然 DOM 渲染到了別處，但 Vue 組件樹中的父子關係不變，provide/inject 照常工作
3. **Suspense 要求 setup 是 async 的或使用了 defineAsyncComponent** —— 普通的 setup 函數不會觸發 Suspense
