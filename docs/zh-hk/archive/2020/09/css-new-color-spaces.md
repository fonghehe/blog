---
title: "CSS 新色彩空間 Lab/LCH 探索：落地路徑與實戰建議"
date: 2020-09-14 11:27:45
tags:
  - CSS
readingTime: 3
description: "最近在做設計系統的顏色體系，發現 CSS 顏色規範正在從 sRGB 向更廣的色彩空間擴展。Chrome 已經開始實驗性支援 `lab()`、`lch()`、`color()` 函數。雖然目前還不能用於生產，但作為前端工程師需要了解這些即將到來的變化。"
wordCount: 606
---

最近在做設計系統的顏色體系，發現 CSS 顏色規範正在從 sRGB 向更廣的色彩空間擴展。Chrome 已經開始實驗性支援 `lab()`、`lch()`、`color()` 函數。雖然目前還不能用於生產，但作為前端工程師需要了解這些即將到來的變化。

## 為什麼需要新的色彩空間

sRGB 能表示的顏色範圍（色域）有限。現在越來越多的設備支援 Display P3 廣色域（所有 Apple 設備、高端顯示器），如果 CSS 隻用 sRGB，就無法充分利用這些硬件的能力。
...
LCH 特別適合構建設計系統的色彩梯度，因為隻需調整亮度（L）就能得到一致的亮暗色階：
...
- `color()` 函數支持指定色域，可以利用 Display P3 廣色域
- 目前瀏覽器支持度有限，建議用 `@supports` 做漸進增強
