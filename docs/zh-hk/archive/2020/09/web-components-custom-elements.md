...
- **Shadow DOM**：隔離的 DOM 子樹，樣式不外泄、不受外部影響
- **HTML Templates**：`<template>` 標籤，惰性解析的模板

## 創建第一個 Custom Element
...
Web Components 的價值在於**框架無關性**。越來越多的設計系統（如 Adobe Spectrum、IBM Carbon）開始提供 Web Components 版本。即使你的日常工作在 Angular 或 React 框架內，理解 Web Components 也有助於你設計更通用的組件邊界。
