...
```typescript
// Vuex 4（TypeScript 支持麻煩）
const store = useStore();
store.commit("user/SET_USER", user); // 字符串類型不安全
store.dispatch("user/login", credentials); // 返回值類型不確定
const name = store.getters["user/displayName"] as string; // 需要手動斷言

// Pinia（TypeScript 友好）
const userStore = useUserStore();
userStore.user = user; // 直接賦值，有類型檢查
await userStore.login(credentials); // 返回類型自動推導
const name = userStore.displayName; // 類型正確
```

## 小結

- Pinia 去掉了 mutations，直接在 actions 裏改 state，更簡單
- 對 TypeScript 友好，類型自動推導
