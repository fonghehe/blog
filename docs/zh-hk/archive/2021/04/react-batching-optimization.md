---
title: "React 18 自動批處理與 Transition：落地路徑與實戰建議"
...
React 17 及之前，隻有 React 事件處理器中的多次 `setState` 會被批處理。在 `setTimeout`、`Promise` 回調、原生事件中則不會：
...
效能提升直接且無需改代碼。唯一的例外是你需要強製同步更新，用 `flushSync`：
