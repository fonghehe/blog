...
以前的前端：寫 HTML → 寫 CSS 類 → 寫 CSS 規則（來回切換文件）

Tailwind：把樣式寫在 HTML 裏，不需要切換文件
...
<!-- 還需要去 CSS 文件定義 .card 和 .card-title -->
...
之前的問題：Tailwind 的 CSS 文件很大（未使用的類很多），生產要用 PurgeCSS 清理。

JIT：根據源碼按需生成 CSS，只生成用到的類，任意值也支持。
...
適合：後台管理系統、營銷頁、文檔站。不適合：需要大量動畫的展示型項目。

## 小結

- Tailwind 3.0 JIT：按需生成 CSS，支持任意值，無需 PurgeCSS
- 動態類名要用完整字符串，否則 JIT 掃描不到
- 組件封裝 + Tailwind：複用靠組件，而不是靠 `@apply`