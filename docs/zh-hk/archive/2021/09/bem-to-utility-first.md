...
BEM（Block-Element-Modifier）我們用了 4 年，解決了命名衝突問題，但隨着項目變大，新問題逐漸暴露：
...
/* 這些 flex、padding、margin 在無數組件中重複 */
```

核心問題是：CSS 文件膨脹、佈局樣式重複、命名心智負擔高。
...
- **組件的結構樣式**：用 BEM，保持語義
- **顏色和字體微調**：用 utility classes

## 用 CSS 變量統一 Design Token
...
我們用 Windi CSS（兼容 Tailwind 的 API）作為 utility 工具，配合 Vite 使用：
...
- Design Token 用 CSS 變量管理，不管用什麼方法論都需要
- Windi CSS / Tailwind JIT 讓 Utility-First 方案的運行時開銷幾乎為零
