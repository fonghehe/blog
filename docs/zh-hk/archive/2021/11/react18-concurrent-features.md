...
```javascript
// React 18：Suspense 支持服務端渲染（SSR）
// 頁面可以分段流式渲染，不需要等所有數據準備好
...
// 流式 SSR：先發送 HTML 骨架，數據準備好了再流式填充
```
...
// 生成唯一 ID（服務端客户端一致，解決 SSR hydration 不匹配問題）
...
// 但在 React 18 中，ReactDOM.render 仍然可用（兼容模式，不開啓併發特性）
...
- Automatic Batching 是 React 18 最實際的改進，不需要改代碼
- useTransition：區分緊急/非緊急更新，保持 UI 響應
- Suspense SSR：流式渲染，用户更快看到內容
