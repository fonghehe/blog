...
1. **文件體積**：`tailwind.css` 生成的文件很大，即使用了 purge
...
差距非常明顯。UnoCSS 是即時生成（基於字符串匹配），不需要解析 AST。

## 與組件庫配合

```typescript
// uno.config.ts - 為組件庫定義規則
export default defineConfig({
  rules: [
    // 組件前綴規則
    [/^cp-btn-(\w+)$/, ([, type]) => ({
...
UnoCSS 的定位很巧妙：不和 Tailwind 搶用户，而是提供底層引擎。Tailwind 的規則可以跑在 UnoCSS 上（preset-wind），但 UnoCSS 還有更多可能性——圖標、屬性化、自定義規則。對於需要高度定製的團隊項目，UnoCSS 是更好的選擇。