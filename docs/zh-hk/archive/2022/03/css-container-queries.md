...
以前用媒體查詢，隻能根據瀏覽器視口來決定樣式——但這裏的卡片組件需要根據父容器寬度來決定佈局。
...
Container Queries 解決了組件化開發中最基本的需求：讓組件根據自己的容器響應式調整佈局，而不是依賴全局視口。這是 CSS 響應式設計從「頁面級」到「組件級」的進化。現在主流瀏覽器都已支援，是時候在組件庫中用起來了。