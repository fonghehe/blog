...
tags:
  - 前端
readingTime: 2
description: "Astro 1.0 正式發佈了。它的核心理念很明確：默認輸出零 JavaScript，隻在需要交互的地方加載 JS。對於內容型網站（博客、文檔、營銷頁），這個方案比 React/Vue 全家桶高效得多。"
wordCount: 331
---

Astro 1.0 正式發佈了。它的核心理念很明確：默認輸出零 JavaScript，隻在需要交互的地方加載 JS。對於內容型網站（博客、文檔、營銷頁），這個方案比 React/Vue 全家桶高效得多。
...
頁面大部分是靜態 HTML，隻有標記為「Island」的組件才會加載 JS 並變成可交互的。
...
靜態構建輸出的是純 HTML + CSS + 極少量 JS。我們博客項目構建後，首頁 JS 體積隻有 2KB（React 做的搜尋組件）。
...
Astro 不是要替代 React/Vue，而是填補了一個空白：內容型網站不需要 SPA 的複雜度。默認零 JS、Islands Architecture、混合框架支援——這些特性讓它成為文檔站和博客的最佳選擇。