...
TypeScript 4.7 終於正確理解 `type: "module"` 和 `exports` 字段了。
...
`module: "NodeNext"` 讓 TypeScript 按照 Node.js 的 ESM 解析規則來處理模塊。
...
// theme1.red 的類型是 string | number[]（太寬了）
...
// theme2.red 的類型是 string ✅
// theme2.green 的類型是 number[] ✅
// 同時保證整體符合 Theme 結構
```
...
TypeScript 4.7 的 ESM 支持讓雙包（CJS + ESM）發佈變得可行。4.8 的類型收窄改進減少了類型斷言的使用。建議在新項目中使用 `module: "NodeNext"`，為 ESM 遷移做準備。