---
title: "Angular 15 發佈：Directive Composition API 與 NgOptimizedImage"
...
這是 Angular 15 最重要的新特性。`hostDirectives` 允許一個組件/指令**內嵌**其他指令的行為，實現類似 mixin 的效果：
...
@Component({
  ...
  hostDirectives: [
    DraggableDirective,
    {
      ...
      inputs: ["appTooltip: tooltip"], // 重新映射 input 名稱
    },
    {
...
@Component({
  ...
  template: `
    <!-- 使用 ngSrc 替代 src，Angular 自動優化 -->
    <img ngSrc="hero.jpg" width="800" height="600" priority />
    <!-- priority: 關鍵圖片（LCP），自動添加 preload link -->
...
Angular 15 大幅改善了錯誤信息，特別是模板中的錯誤：
...
Angular 15 的核心貢獻是：Directive Composition API 填補了長期缺失的指令複用能力，NgOptimizedImage 讓圖片優化變成一鍵式操作，Standalone APIs 正式穩定則標誌着"無 NgModule"時代正式到來。對 Angular 團隊來説，2022 年的兩個版本（14、15）讓框架重新煥發了活力。