...
tags:
  - Angular
readingTime: 2
description: "Angular 15 讓 Standalone APIs 正式穩定後，很多團隊開始考慮遷移現有項目。Angular 提供了自動化遷移工具，但瞭解遷移的每個步驟和潛在問題仍然必要。這篇文章梳理完整的遷移路徑。"
wordCount: 237
---

Angular 15 讓 Standalone APIs 正式穩定後，很多團隊開始考慮遷移現有項目。Angular 提供了自動化遷移工具，但瞭解遷移的每個步驟和潛在問題仍然必要。這篇文章梳理完整的遷移路徑。
...
# 第一步：將組件/指令/管道標記為 standalone
ng generate @angular/core:standalone
# 選擇 "Convert all components, directives and pipes to standalone"
# 這會給每個組件加上 standalone: true，並將 NgModule 的 imports 移到組件的 imports
...
### 1. 組件依賴從 Module 移到 Component

```typescript
// 遷移前：組件通過 NgModule 獲得依賴
@NgModule({
...
bootstrapApplication(AppComponent, {
  providers: [
    ...
    provideHttpClient(
      withInterceptors([authInterceptor]), // 函數式攔截器
    ),
...
遷移中最麻煩的通常是 `SharedModule`（包含大量公共組件）：
...
// 遷移後：每個組件自包含，可以單獨按需導入
// 不再需要 SharedModule，直接在需要的地方導入具體組件：
@Component({
  standalone: true,
  imports: [ButtonComponent, CardComponent],  // 按需導入
  ...
})
...
// 問題 2：懶加載的 standalone 組件 vs 懶加載路由組
// 單個 standalone 組件懶加載
...
階段 2：逐步遷移葉子組件（無子組件依賴的組件）
階段 3：遷移 SharedModule 中的組件
...
Angular 15 的 Standalone 遷移工具降低了遷移成本，但對於大型項目仍需要謹慎規劃。最重要的是理解：Standalone 不是"新功能"，而是 NgModule 的替代——兩者可以共存，遷移可以漸進進行。遷完之後代碼量通常減少 20-30%，架構也更直觀。