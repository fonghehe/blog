...
在 PPR 模式下，`StaticHero` 部分在構建時就生成了 HTML 和 RSC Payload，只有 `PersonalizedRecommendations` 這個 Suspense 邊界內的內容在運行時流式填充。
...
限制也很明顯：PPR 要求頁面可以清晰地劃分為靜態和動態部分。如果整個頁面都是高度個性化的（如用户後台），PPR 收益有限。
