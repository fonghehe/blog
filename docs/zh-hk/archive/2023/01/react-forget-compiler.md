...
function ProductCard({ product, onAddToCart }: Props) {
  ...
  // style 對象只有在 tagColor 變化時才重新創建
  const style = useMemo(() => ({ color: tagColor }), [tagColor])

  // 事件處理器：只有在 onAddToCart 和 product.id 變化時才重新創建
...
如果 Forget 成熟，開發者的心智模型會發生轉變：從"我需要考慮性能"到"我只需要寫正確的 React 代碼"。
...
- Forget 不會改變組件行為，只優化不必要的重新計算和引用變化
