...
`useTransition` 和 `startTransition` 讓 React 知道哪些更新可以被中斷。典型場景：輸入框實時過濾大量數據。
...
React 18 的 `Suspense` 配合 Server Components 和 lazy loading，是一個統一的異步邊界。

```tsx
import { Suspense } from 'react'

// 場景一：數據獲取（配合 Server Components 或 use() 鈎子）
  ...
  return (
    <div>
      ...
      <Suspense fallback={<ProfileSkeleton />}>
        {/* UserProfile 是異步 Server Component */}
...
`Suspense` 的關鍵是"瀑布流"加載——多個 Suspense 邊界獨立工作，先到先渲染，而不是等所有數據都就緒。

## use() 鈎子：讀取 Promise 和 Context

React 18.x 實驗性 API 中的 `use()` 鈎子，可以在條件語句和循環中讀取 Promise 和 Context，這是 `useContext` 和 `await` 做不到的。
...
// 條件讀取 Context — useContext 不能放在 if 裏，use() 可以
...
React 18 將批量更新從 React 事件處理器擴展到了所有上下文，包括 `setTimeout`、`Promise` 回調、原生事件。
...
- `useTransition` 區分緊急和非緊急更新，搜索過濾是最典型的用例
- `Suspense` 是統一的異步邊界，支持數據獲取和代碼分割，配合流式渲染效果最佳
- `use()` 鈎子可以在條件語句中讀取 Promise 和 Context，比 `useContext` 更靈活
- React 18 自動批量更新覆蓋了所有異步場景，`flushSync` 是 escape hatch
