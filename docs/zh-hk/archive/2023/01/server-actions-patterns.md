---
title: "Server Actions 設計模式：落地路徑與實戰建議"
date: 2023-01-11 16:06:36
tags:
  - 前端
readingTime: 3
description: "Server Actions 不隻是替代 API 路由的語法糖，它改變了數據流的組織方式。在實際項目中，我們需要一套成熟的設計模式來管理校驗、錯誤處理、權限控製和狀態同步。這篇文章總結了我在生產環境中驗證過的幾種模式。"
wordCount: 432
---

Server Actions 不隻是替代 API 路由的語法糖，它改變了數據流的組織方式。在實際項目中，我們需要一套成熟的設計模式來管理校驗、錯誤處理、權限控製和狀態同步。這篇文章總結了我在生產環境中驗證過的幾種模式。
...
有時一個用户操作需要更新多張表。Server Action 天然支援數據庫事務，不需要前端發多次請求。
...
- 每個 action 都要鑑權，不要依賴 middleware 做權限控製