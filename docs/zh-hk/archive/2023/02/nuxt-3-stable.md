---
title: "Nuxt 3 正式版完整實踐指南：特性解讀與遷移建議"
date: 2023-02-01 09:31:40
tags:
  - Nuxt.js
readingTime: 3
description: "Nuxt 3 基於 Vue 3 + Nitro 服務引擎 + Vite，提供了全棧 Vue 開發的最佳實踐。相比 Nuxt 2，它在類型安全、效能、部署靈活性方面都有質的提升。本文從實際項目出發，介紹 Nuxt 3 的核心特性。"
wordCount: 456
---
...
Nuxt 3 延續了約定式目錄結構，但做了簡化。`composables/`、`components/`、`layouts/` 目錄中的檔案會被自動導入，不需要手動 `import`。
...
Nuxt 3 的服務端引擎 Nitro，支援從同一個代碼庫部署到 Node.js、Cloudflare Workers、Vercel、AWS Lambda 等多種平臺。
...
Nuxt 3 的部署設定極其靈活，通過 `nitro` 設定指定目標平臺：
...
- Nuxt 3 基於 Vue 3 Composition API，自動導入機製大幅減少樣板代碼
- `useFetch` 是數據獲取的核心 API，內置緩存、loading、error 處理
- Nitro 服務引擎支援多平臺部署，同一套代碼可部署到 Node/Edge/Serverless
