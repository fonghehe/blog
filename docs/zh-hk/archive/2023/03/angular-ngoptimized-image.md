---
title: "Angular NgOptimizedImage 深度實踐：LCP 優化從理論到落地"
...
| 指標       | 優化前 | 優化後（NgOptimizedImage + Imgix） |
| 
---------- | ------ | ---------------------------------- |
| LCP        | 3.8s   | 1.6s                               |
| CLS        | 0.12   | 0.01                               |
...
`NgOptimizedImage` 將圖片優化的最佳實踐內置到框架層——開發者不再需要手動處理 srcset、preload hint、lazy loading。對 LCP 指標有要求的項目（電商、內容類），配合 CDN Loader 使用，是目前 Angular 中改善 Core Web Vitals 成本最低的方案。