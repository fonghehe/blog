---
title: "組件庫自動化測試體系：從覆蓋率到質量度量"
date: 2023-05-15 16:44:21
tags:
  - 前端
readingTime: 2
description: "作為組件系統 owner，測試覆蓋率隻是起點。真正重要的是測試能攔住多少線上問題。分享一下我們組件庫的測試體系設計。"
wordCount: 314
---

作為組件系統 owner，測試覆蓋率隻是起點。真正重要的是測試能攔住多少線上問題。分享一下我們組件庫的測試體系設計。
...
E2E 隻覆蓋最核心的交互路徑，不要寫太多——維護成本高、速度慢。
...
- 組件測試用 `getByRole` 而非 `getByTestId`，測試更貼近用户視角
- Storybook + Chromatic 做視覺迴歸，是組件庫特有的測試手段
