...
Vite 5 使用的 esbuild 版本更新，對 CSS 處理和 ESM 解析做了優化。更重要的是，Vite 團隊正在開發 Rolldown（基於 Rust 的 Rollup 替代），Vite 5 是為這個過渡做準備。
...
Vite 5 引入了 Environment API 的概念，為 SSR 和 edge runtime 提供更好的支持：
...
// 3. import.meta.glob 的 Eager 默認值變化
// Vite 5: 默認是 lazy（動態 import）
...
提升不算巨大，但在大型項目和 monorepo 中更明顯。

## 小結

- Vite 5 是一次"現代化清理"，去掉了對舊 Node 版本和過時 API 的支持
- 底層引擎升級，為未來的 Rolldown 遷移做準備
- Environment API 為 SSR 和 edge 場景打基礎
- 從 Vite 4 遷移成本低，大部分項目幾小時搞定
