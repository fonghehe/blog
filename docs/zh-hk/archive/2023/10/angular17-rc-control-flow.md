...
Angular 的結構指令（`*ngIf`、`*ngFor`）是歷史遺留設計，需要 `CommonModule` 或單獨導入，語法也不直觀。Angular 17 引入了語言級別的控制流語法：
...
Angular 17 的 `@for` **強制要求** `track` 表達式（而 `*ngFor` 的 `trackBy` 是可選的），這是一個重要改進：
...
這是 Angular 17 最激動人心的特性。`@defer` 讓組件/模板塊可以**懶加載**——不只是懶加載路由，而是細粒度到模板中任意一塊內容：
...
Angular 17 是近年來對模板語法改動最大的版本。新控制流語法更直觀，強制 `track` 減少了列表渲染的性能陷阱；`@defer` 是前端懶加載的新範式——從"路由懶加載"升級到"內容塊懶加載"。正式版預計 11 月 8 日發佈，RC 階段可以開始在項目中測試遷移。