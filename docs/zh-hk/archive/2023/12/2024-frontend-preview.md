---
title: "2024 前端展望：React 編譯器、Angular Signals 穩定與 AI 輔助編程元年"
date: 2023-12-30 10:22:19
tags:
  - 前端
readingTime: 2
description: "2023 年是前端生態\"技術落地\"的一年：Next.js 13 App Router 進入生產，Angular 17 帶來了革命性的範本語法，Bun 1.0 正式發佈，AI 輔助編程從新奇變成日常工具。站在年末，我們預判 2024 年的關鍵走向。"
wordCount: 355
---
...
function ExpensiveList({ items, filter }) {
  const filtered = useMemo(() => items.filter(filter), [items, filter]);  // 手動緩存
  const handleClick = useCallback((id) => onDelete(id), [onDelete]);      // 手動緩存
...
// 開發者不再需要手寫這些優化——編譯器比人更聰明
function ExpensiveList({ items, filter }) {
  const filtered = items.filter(filter);  // 編譯器自動緩存
...
// 1. Signal-based 組件（OnPush 默認，Zone.js 可選）
@Component({
  changeDetection: ChangeDetectionStrategy.OnPush,  // 或將成為默認
  // ...
})
...
Vite 5 於 2023 年 11 月發佈，2024 年繼續作為前端構建標準：
...
// TS 5.4 預期：NoInfer<T> 工具類型
function createStore<T>(initial: T, update: (state: NoInfer<T>) => NoInfer<T>) {
  // NoInfer 防止從參數推斷類型，避免意外的類型擴展
}

// TS 5.5 預期：改進的類型謂詞推斷
// 過濾 null 時不再需要手寫 is 謂詞
const filtered = items.filter((item) => item !== null);
// filtered 自動推斷為非 null 類型（而非 typeof items[0] | null）
```

## AI 輔助編程：從工具變成工作流

2023 年 AI 編程工具爆發（GitHub Copilot、Cursor、Cody），2024 年預計：

```
進化方向：
- 從"單文件補全"到"多文件上下文理解"
- 從"代碼生成"到"需求理解 → 方案拆解 → 代碼實現"
- 測試生成自動化（寫函數 → AI 自動生成測試用例）
- 代碼 Review 輔助（安全漏洞、性能問題自動檢測）

實際影響：
- 模板代碼（CRUD、表單）生成效率提升 3-5x
- 單元測試覆蓋率可以更容易提高到 80%+
- API 文檔和註釋不再是負擔
```
...
2024 年前端的關鍵詞：**編譯器驅動優化**（React Compiler、Angular 編譯器）、**AI 深度融入工作流**、**構建工具 Rust 化**（Rolldown、oxc）、**框架 API 收斂**（各框架完成 2022-2023 年引入的大特性的穩定化）。對開發者來説，2024 年最值得投入的是：熟悉 Angular Signals/React Compiler 的新範式，以及認真將 AI 工具融入日常開發流程。