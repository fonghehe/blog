...
tags:
  - TypeScript
readingTime: 2
description: "TypeScript 5.0 發佈了正式的裝飾器標準（TC39 Stage 3），和之前實驗性裝飾器（`experimentalDecorators`）有幾處重要區別。"
---

TypeScript 5.0 發佈了正式的裝飾器標準（TC39 Stage 3），和之前實驗性裝飾器（`experimentalDecorators`）有幾處重要區別。
...
const user = new User();
user.name = "張三"; // 打印：name 從  變為 張三
user.age = 25; // 打印：age 從 0 變為 25
```
...
兩套裝飾器**不兼容**。如果你用了 `class-validator`、`typeorm`、`reflect-metadata`，這些庫還依賴舊裝飾器，暫時不要遷移。
...
async function processFile() {
  ...
  // 函數結束時自動調用 file[Symbol.asyncDispose]()
  return file.read();
}
```
