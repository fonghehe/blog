---
title: "Biome 完整工作流：從遷移、CI 集成到團隊規範"
...
階段 2：Biome 作為 linter，和 ESLint 並行運行
...
關鍵一步：格式化現有代碼並做一次大提交，避免 diff 污染後續 PR：
...
Biome 的規則和 ESLint 不是 1:1 映射。有些 ESLint 插件規則（如 `eslint-plugin-react` 的某些規則）Biome 還不支持。
...
VS Code 的 Biome 擴展比 ESLint 擴展年輕，偶爾會有卡頓。解決方案是重啓 LSP server。
