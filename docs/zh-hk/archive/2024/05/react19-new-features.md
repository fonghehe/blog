...
tags:
  - React
readingTime: 2
description: "React 19 Beta 發佈，看來正式版很快了。整理一下主要新特性。"
---

React 19 Beta 發佈，看來正式版很快了。整理一下主要新特性。
...
// 接受 Promise（比 await 更靈活，可以在條件分支裏用）
function UserProfile({ userPromise }: { userPromise: Promise<User> }) {
  const user = use(userPromise); // 如果 Promise 未完成，拋出 Promise（Suspense 接住）
  return <div>{user.name}</div>;
}

function App() {
  const userPromise = fetchUser(userId); // 在組件外發起請求（不阻塞渲染）
...
// 也可以讀 Context（比 useContext 更靈活，可以在條件語句裏用）
...
async function ProductPage({ id }: { id: string }) {
  const product = await getProduct(id); // 服務端直接訪問數據

  return (
    <div>
      <h1>{product.name}</h1>
      <ProductGallery images={product.images} /> {/* Server Component */}
      <AddToCart productId={id} /> {/* 'use client' 組件 */}
    </div>
  );
}
```
...
- `useFormStatus`：子組件感知父表單狀態，不用 prop drilling
- `useOptimistic`：內置樂觀更新模式
