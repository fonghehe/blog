---
title: "Vue 3.5：響應式系統重寫與性能飛躍"
date: 2024-06-10 10:00:00
tags:
  - Vue
readingTime: 2
description: "Vue 3.5 發佈，這是 Vue 3 繼 3.4 之後的又一個重要版本。最核心的變化是響應式系統的完全重寫，帶來了顯著的內存和性能改善。"
---

Vue 3.5 發佈，這是 Vue 3 繼 3.4 之後的又一個重要版本。最核心的變化是響應式系統的完全重寫，帶來了顯著的內存和性能改善。

## 響應式系統重寫

Vue 3.5 對 `reactive()`、`ref()`、`computed()` 的底層實現做了重構，核心目標是減少內存佔用。
...
重寫的重點是優化了 Proxy handler 的實現，減少了中間對象的創建。

### 對實際項目的影響

我們有一個數據密集型的管理後台，頁面經常掛載 1000+ 個響應式節點：

```vue
<script setup lang="ts">
// 以前用 shallowRef 處理大數組避免響應式開銷
const largeList = shallowRef<Item[]>([]);

// Vue 3.5 之後，普通 ref 也足夠輕量
const largeList = ref<Item[]>([]);

// 1000 個對象的響應式包裝，內存從 ~8MB 降到 ~3.5MB
</script>
```
...
新增的 `useId` 組合式函數，生成服務端渲染安全的唯一 ID：
...
Teleport 組件新增 `defer` 選項，延遲到 DOM 就緒後再傳送：
...
2. 大數組場景可以去掉 `shallowRef`，改用普通 `ref`
3. 組件 ID 生成邏輯可以遷移到 `useId`
4. 測試 SSR 水合是否正常

## 小結

- 響應式系統重寫：內存佔用降低約 56%，大型數據場景收益明顯
- `defineModel` 穩定：簡化 v-model 雙向綁定
