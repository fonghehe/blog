...
**序列化邊界要注意**。Server Component 向 Client Component 傳遞 props 時，隻能傳遞可序列化的數據。Date 對象、Map、Set 都不能直接傳。

**"use client" 不是整個組件樹**。標記了 `"use client"` 的組件仍然可以在服務端渲染，隻是它的子組件會在客户端 hydrate。
...
- 關鍵是控製好 Server/Client 邊界，最小化客户端代碼
