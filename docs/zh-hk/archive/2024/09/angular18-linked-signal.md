---
title: "Angular 18.2 linkedSignal：響應式依賴信號的新原語"
date: 2024-09-11 10:00:00
tags:
  - Angular
readingTime: 2
description: "Angular 18.2 於 2024 年 8 月 14 日發佈，引入了 `linkedSignal()`——一個實驗性的新 Signal 原語。它解決了 `computed()` 和可寫 `signal()` 之間長期存在的一個痛點：**如何創建一個既能從外部源派生初始值，又允許本地修改的 Signal**。"
wordCount: 297
---

Angular 18.2 於 2024 年 8 月 14 日發佈，引入了 `linkedSignal()`——一個實驗性的新 Signal 原語。它解決了 `computed()` 和可寫 `signal()` 之間長期存在的一個痛點：**如何創建一個既能從外部源派生初始值，又允許本地修改的 Signal**。
...
`linkedSignal()` 支持訪問前一個值，實現更復雜的邏輯：
...
linkedSignal()  ← 新增
  ...
  用途：有"默認值來自外部，但用户可覆蓋"語義的狀態
```

## 常見應用場景

```typescript
// 場景 1：表單字段的默認值來自 props，用户可修改
...
`linkedSignal()` 填補了 Angular Signals 原語體系中的一個重要空缺——"可變的派生信號"。它讓"選中項跟隨列表變化重置，但用户操作優先"這類常見 UI 模式有了優雅的表達方式。配合 Angular 18.1 已穩定的 Signal API，Angular 的響應式模型正在變得越來越完整。
