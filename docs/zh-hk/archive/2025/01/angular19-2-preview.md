...
```html
<!-- 新 Signal Forms 模板綁定（草案，API 未最終確定）-->
...
Angular 19.2 探索將 `@defer` 與路由懶加載結合：
...
// 19.2 探索：路由組件內的 @defer 塊也參與路由預加載策略
// 即：進入路由時，只加載可見內容；滾動到下面時按需加載
```

## Server-Side Rendering 性能改進

Angular 19.2 改進了 SSR 的 Transfer State 機制：

```typescript
// 改進前：所有 HTTP 請求結果都序列化到 Transfer State（可能很大）
// 改進後：支持選擇性 Transfer（只傳輸需要的數據）
...
Angular DevTools 在 19.2 中新增 Signal 追蹤視圖：

```
Angular DevTools 19.2 新增功能：
- Signal 依賴圖可視化：查看哪些 Signal 影響了哪些 computed/effect
- Incremental Hydration 狀態監控：看哪些 @defer 塊已水合，哪些還未
- Zoneless 性能分析：對比有無 zone.js 的變更檢測次數
```
...
# 查看 Signal Forms API（發佈後）
ng add @angular/forms@19.2
```

## 總結

Angular 19.2 最值得期待的是 Signal-based Forms——這是長期以來 Angular 表單系統最大的現代化嘗試。一旦進入開發者預覽，結合 Signal Inputs/Outputs/Queries，Angular 組件的整個 API 將完全脱離"裝飾器 + Observable"的歷史包袱。今年是 Angular 表單系統的轉型之年，值得密切關注。
