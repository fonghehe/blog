---
title: "React 20 Compiler 穩定版發佈：特性解讀與遷移建議"
date: 2025-01-01 10:55:32
tags:
  - React
readingTime: 3
description: "React 20 Compiler（原 React Forget）終於從 RC 進入穩定版。這個編譯器徹底改變了 React 效能優化的方式——你不再需要手動寫 `useMemo`、`useCallback`、`React.memo`，編譯器會自動幫你搞定。"
wordCount: 631
---

React 20 Compiler（原 React Forget）終於從 RC 進入穩定版。這個編譯器徹底改變了 React 效能優化的方式——你不再需要手動寫 `useMemo`、`useCallback`、`React.memo`，編譯器會自動幫你搞定。
...
最大的好消息：React 20 Compiler 是向後相容的。你的現有代碼不需要任何改動就能跑。但有幾個行為變化值得注意：
...
- React 20 Compiler 穩定版徹底消除了手動記憶化的需求，效能優化由編譯器自動完成
