...
tags:
  ...
  - CSS
readingTime: 2
description: "Angular 20 讓 Zoneless 正式穩定，但實際遷移並不是改一行設定就完事。本文記錄了將一箇中型 Angular 項目（約 80 個組件）從 zone.js 遷移到 Zoneless 的完整過程，分享遇到的坑和解決方案。"
wordCount: 270
---

Angular 20 讓 Zoneless 正式穩定，但實際遷移並不是改一行配置就完事。本文記錄了將一箇中型 Angular 項目（約 80 個組件）從 zone.js 遷移到 Zoneless 的完整過程，分享遇到的坑和解決方案。

## 遷移準備：檢測代碼中的 zone.js 依賴

在去掉 zone.js 之前，先搞清楚項目中有哪些地方隱式依賴了它：
...
```typescript
// ❌ 模式 1：在 setTimeout/setInterval 回調中修改非 Signal 狀態
...
```typescript
// ❌ 模式 2：直接訂閲 RxJS Observable 並修改普通屬性
...
階段 1（1-2周）：全面改用 OnPush + 清除 zone 依賴
  ① 所有組件加上 changeDetection: ChangeDetectionStrategy.OnPush
  ② 將普通屬性改為 signal()
  ③ 用 toSignal() 包裹 Observable
  ④ 用 takeUntilDestroyed() 替代手動 unsubscribe

階段 2（1周）：並行測試 Zoneless 模式
  ① 在開發環境單獨創建 Zoneless 構建
  ② 運行 E2E 測試，找出因 zone.js 依賴而失敗的用例
  ③ 修復所有問題
...
我們項目遷移到 Zoneless 後的實際數據：

```
包體積：減少 13KB（gzip），首屏加載提升約 50ms（弱網）
變更檢測次數：減少約 60%（無 zone 的全局攔截）
LCP 提升：平均 120ms（減少了 JS 執行時間）
內存佔用：減少約 8%（無 zone.js 的 patch 開銷）
```

## 總結

Zoneless 遷移的核心是"把所有可變狀態都 Signal 化"。這本來就是 Angular 17+ 推薦的最佳實踐，所以如果你的新項目一開始就用 Signal API，遷移到 Zoneless 幾乎是零成本的。難點在於老項目——大量使用 `subscribe` + 直接賦值的代碼需要系統性改造。分階段遷移（OnPush → Zoneless）比一次性切換風險更低。
