...
"P75 的 LCP 是 4.2s，目標是 2.5s 以內，
 通過 Chrome DevTools 分析，瓶頸在圖片加載和 JS 解析"
```

關鍵指標：

```
Core Web Vitals（用户體驗）：
  LCP（最大內容繪製）< 2.5s：首屏主要內容加載完成
  INP（交互到下一幀繪製）< 200ms：取代了 FID
  CLS（累積佈局位移）< 0.1：頁面不要跳來跳去

業務指標：
  TTFB（首字節時間）：服務器響應速度
  TTI（可交互時間）：頁面能響應用户操作的時間
  Bundle Size：JS 包大小
```
...
INP 取代了 FID，測量的是用户交互到下一幀渲染的時間。

```javascript
// 長任務會阻塞主線程，導致 INP 差
// 用 scheduler.yield() 讓出主線程（Chrome 115+）
...
function Page() {
  ...
  return (
    <>
      <button onClick={() => setEditorOpen(true)}>打開編輯器</button>
      {editorOpen && (
        <Suspense fallback={<div>加載編輯器...</div>}>
          <MarkdownEditor />
        </Suspense>
      )}
    </>
...
6. 監控迴歸：Lighthouse CI 設置閾值，變差了 CI 失敗
```
...
- LCP：preload 關鍵圖片，使用現代格式，不要懶加載首屏圖
- INP：避免長任務，用 scheduler.yield() 和 startTransition 讓出主線程
- Bundle Size：路由級別代碼分割 + 條件懶加載
- 把性能測試加入 CI，防止迴歸
