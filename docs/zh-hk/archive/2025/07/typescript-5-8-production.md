...
### 1. --erasableSyntaxOnly：為 Node.js 原生 TS 支援優化

Node.js 22+ 支援直接運行 `.ts` 檔案（不生成 .js），但隻支援"可擦除語法"（不允許 `enum`、`namespace` 等運行時語法）。TS 5.8 新增 `--erasableSyntaxOnly` 選項來檢測違規：
...
```typescript
// TS 5.8 之前：提取函數返回類型的 Promise 內層類型需要巢狀 infer
...
### 問題 3：template literal 類型的遞歸限製

```typescript
// 深層巢狀的 template literal 類型會觸發 TS 的遞歸限製
...
// 實踐：限製深度或使用運行時方案（zod、yup）替代複雜的類型推導
```
