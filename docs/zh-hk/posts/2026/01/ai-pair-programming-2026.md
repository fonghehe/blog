---
title: "AI 結對編程 2026 工作流"
date: 2026-01-19 10:00:00
tags:
  - 工程化
readingTime: 3
description: "AI 結對編程已經超越了\"AI 寫代碼，人看代碼\"的簡單模式。2026 年的成熟工作流是分工明確的協作：人負責架構思考和業務邏輯，AI 負責實現細節和模式化代碼。本文分享我日常使用的 AI 結對編程工作流。"
wordCount: 470
---

AI 結對編程已經超越了"AI 寫代碼，人看代碼"的簡單模式。2026 年的成熟工作流是分工明確的協作：人負責架構思考和業務邏輯，AI 負責實現細節和模式化代碼。本文分享我日常使用的 AI 結對編程工作流。

## 駕駛員與領航員：角色分配

傳統結對編程中人和人的角色切換，在人機協作中變成了"什麼時候該讓 AI 上"。我的原則是：探索性編碼由人主導，實現性編碼讓 AI 上。
...
const workflowRules = {
  ...
  aiDrives: [
    ...
    '類型定義和接口文檔生成',
    '重複性重構（重命名、文件遷移）',
    'Bug 修復模式代碼（try-catch、null check）',
  ],

  // 關鍵規則：AI 每次只能在一個模塊內工作
  // 跨模塊變更必須由人發起
...
我使用 VS Code + Claude Code 擴展的組合。核心技巧不是"讓 AI 補全"，而是用 structured prompts 引導 AI 理解你想做什麼。
...
// 第二步：告訴 AI "根據上面的接口實現 useDataTable hook"
...
AI 結對的隱藏價值是幫你理解你不熟悉的代碼。面對一個 3 萬行的遺留系統，讓 AI 幫你畫出調用鏈和數據流。

```typescript
// 實際用法：在終端中對遺留代碼提問
// $ claude "src/legacy/order-processor.ts 中的 processOrder 函數
//   的完整調用鏈是什麼？有哪些外部依賴？哪些分支有副作用？"
...
AI 最大的效率陷阱是"過度對話"。和 AI 聊了 20 輪還沒開始寫代碼，不如自己 10 分鐘寫完。我的經驗是：如果 3 輪對話內 AI 還沒給出可用的方案，就換一個方式——要麼給更精確的 prompt，要麼自己先動手。
...
const efficiencyRules = {
  ...
  metrics: {
    ...
    'ai-time-to-first-usable': '從提問到獲得可用代碼的時間',
...
- 人定接口和架構，AI 做實現和測試，這是最高效的角色分配
- 用 AI 理解遺留代碼的價值被嚴重低估，這是 AI 結對的隱藏殺器
- 警惕"過度對話"陷阱，3 輪沒結果就換策略
- 記錄效率指標，用數據驅動 AI 協作方式的優化
