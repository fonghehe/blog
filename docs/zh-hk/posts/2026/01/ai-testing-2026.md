...
tags:
  - 工程化
readingTime: 4
description: "寫測試用例是前端開發中最枯燥但最重要的工作之一。2026 年，AI 已經能自動生成單元測試、集成測試、甚至端到端測試。但\"自動生成\"不等於\"自動生成好測試\"——垃圾測試比沒有測試更危險，因為它給你虛假的安全感。"
wordCount: 484
---

寫測試用例是前端開發中最枯燥但最重要的工作之一。2026 年，AI 已經能自動生成單元測試、集成測試、甚至端到端測試。但"自動生成"不等於"自動生成好測試"——垃圾測試比沒有測試更危險，因為它給你虛假的安全感。

## 測試生成的正確姿勢

直接讓 AI "給這個組件寫測試"產出的代碼通常是垃圾。好的測試生成需要你提供上下文：用户的實際使用路徑、邊界條件、以及"這個組件最容易出 bug 的地方"。
...
端到端測試是最難自動生成的。因為 AI 需要理解用户旅程、等待異步操作、處理動態內容。關鍵是在 prompt 中描述清楚用户操作序列，而不是讓 AI 自由發揮。

```typescript
// e2e-test-prompt.ts —— 給 AI 的 E2E 測試 prompt 模板
const e2eTestPrompt = `
為以下用户旅程生成 Playwright E2E 測試：

旅程：用户註冊 -> 瀏覽商品 -> 加入購物車 -> 結算

關鍵斷言點：
1. 註冊成功後跳轉到首頁，右上角顯示用户名
2. 商品列表加載完成後顯示至少 12 個商品卡片
3. 加入購物車後底部購物車角標數字 +1
4. 結算頁面顯示的商品數量和價格與購物車一致

技術細節：
- API 走 mock（msw），不要依賴後端
- 使用 page.waitForResponse 等待數據加載
- 使用 data-testid 定位元素
`;
...
test('完整購物流程', async ({ page }) => {
  ...
  await page.getByTestId('name-input').fill('測試用户');
  ...
  await expect(page.getByTestId('user-name')).toHaveText('測試用户');
...
覆蓋率數字本身沒有太大意義。AI 的價值是分析哪些未覆蓋的代碼路徑是真正有風險的，幫你排優先級。
...
const gaps: CoverageGap[] = [
  ...
  },
  {
    ...
    riskScore: 88,
    reason: '網絡切換時的重連邏輯，涉及定時器和狀態清理',
    suggestedTest: '應模擬網絡斷開->恢復，驗證重連和消息補發',
  },
];
...
測試最大的成本不是寫，而是維護。AI 可以監控測試失敗、自動修復因代碼變更導致的測試斷裂、甚至標記過時的測試。
...
async function autoFixTests() {
  ...
  for (const failure of failures) {
    ...
    const fixPrompt = `
測試文件 ${failure.name} 失敗。
失敗信息：${failure.message}
最近的代碼變更：${diff}
請分析失敗原因並修復測試。如果是代碼變更導致的預期行為變化，
更新測試斷言；如果是迴歸 bug，標記為需要人工處理。
    `;

    // 調用 AI 修復
    // AI 會區分"測試需要更新"和"代碼有 bug"兩種情況
  }
}
```
