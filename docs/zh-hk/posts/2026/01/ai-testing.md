...
直接讓 AI "給這個組件寫測試"產出的代碼通常是垃圾。好的測試生成需要你提供上下文：用戶的實際使用路徑、邊界條件、以及"這個組件最容易出 bug 的地方"。
...
端到端測試是最難自動生成的。因為 AI 需要理解用戶旅程、等待異步操作、處理動態內容。關鍵是在 prompt 中描述清楚用戶操作序列，而不是讓 AI 自由發揮。
...
旅程：用戶註冊 -> 瀏覽商品 -> 加入購物車 -> 結算

關鍵斷言點：
1. 註冊成功後跳轉到首頁，右上角顯示用戶名
...
test('完整購物流程', async ({ page }) => {
  ...
  await page.getByTestId('name-input').fill('測試用戶');
  ...
  await expect(page.getByTestId('user-name')).toHaveText('測試用戶');
