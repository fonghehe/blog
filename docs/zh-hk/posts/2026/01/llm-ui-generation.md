...
tags:
  - 工程化
readingTime: 3
description: "從 Figma 設計稿到可用代碼，這一步曾經是前端最耗時的環節之一。2026 年，LLM 驅動的 UI 生成工具已經成熟到可以理解設計意圖、組件層級、甚至交互狀態。但\"能用\"和\"能上生產\"之間仍然有巨大的鴻溝。"
---

從 Figma 設計稿到可用代碼，這一步曾經是前端最耗時的環節之一。2026 年，LLM 驅動的 UI 生成工具已經成熟到可以理解設計意圖、組件層級、甚至交互狀態。但"能用"和"能上生產"之間仍然有巨大的鴻溝。

## 設計稿到代碼：當前工具鏈對比

市面上主流的三套方案各有優劣。v0.dev 適合快速原型，Claude Artifacts 適合複雜交互組件，GitHub Copilot Workspace 適合和現有項目集成。

```tsx
// 場景：從自然語言描述生成一個完整的 Dashboard 佈局
// 輸入："創建一個 SaaS 數據看板，左側導航欄，頂部搜索+通知，
...
裸生成 HTML 沒意義。高級的做法是讓 LLM 理解你的組件庫 API，生成直接使用現有組件的代碼。關鍵是餵給模型你的組件文檔和用法示例。

```typescript
// llm-ui-config.ts —— 組件庫感知配置
export const componentAwareConfig = {
  // 將組件庫的 props 定義作為 context 注入
  componentDocs: {
    source: './src/components/ui',
    format: 'extracted-props',
    // 提取每個組件的 props 類型、slot、variant
    extractors: ['typescript-docs', 'storybook-meta'],
  },

  // 生成時的約束
  constraints: {
    // 只使用已有的組件，不要生成原生 HTML
  ...
  postValidation: [
    'typescript-check',
    'visual-regression-compare',  // 和設計稿做像素級對比
    'accessibility-audit',
  ],
};
...
AI 生成的 UI 代碼最多隻完成了 60% 的工作。剩下的 40% 是響應式適配、無障礙訪問、邊緣情況處理、以及和業務邏輯的集成。自動化流水線能幫你快速定位問題。
...
靜態 UI 生成是容易的部分。真正的挑戰是讓 AI 理解業務邏輯，生成正確的表單驗證、狀態管理、數據流。我的做法是先用 AI 生成 UI 骨架，再用第二輪 Agent 注入交互邏輯。

```tsx
// 第二輪 Agent 輸入：
// "為上面的 Dashboard 添加：點擊卡片展開詳情抽屜，
//   使用 URL search params 同步選中狀態，
//   抽屜內用 TanStack Table 展示明細數據"
...
- 組件庫感知的生成策略能大幅減少返工，關鍵是有結構化的組件文檔
- 生成後必須經過類型檢查、視覺迴歸、無障礙審計三道關卡
- 交互邏輯建議用第二輪 Agent 注入，分層生成比一次性生成更可靠
- 2026 年最好的工作流：AI 出初稿，人做精修和業務邏輯集成
