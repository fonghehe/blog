...
簡單説：把一個大型前端應用拆分成多個可以**獨立開發、獨立部署**的小應用，在運行時組合成一個整體。
...
```
1. 樣式隔離：各應用樣式不互相污染，需要 CSS scope 策略
2. 狀態共享：用户登錄信息、主題等需要跨應用共享
3. 路由協同：主應用路由 + 子應用路由如何配合
4. 構建複雜度：每個應用需要單獨的 CI/CD pipeline
5. 聯調困難：本地同時跑 N 個應用才能調試
6. 性能：重複加載相同依賴（React 可能被加載多次）
```
...
如果確定要用微前端，2025 年我會推薦 Webpack 5 Module Federation 或 Vite 的 federation 插件：
...
export default {
  plugins: [
    federation({
      ...
      shared: ["react", "react-dom"], // 共享依賴，避免重複加載
    }),
  ],
};
```
...
function App() {
  return (
    <Suspense fallback={<div>加載訂單模塊...</div>}>
      <OrderList />
    </Suspense>
  );
}
