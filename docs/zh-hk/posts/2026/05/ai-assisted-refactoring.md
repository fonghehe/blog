---
title: "AI 輔助重構：大型前端項目的安全演進流程"
date: 2026-05-29 14:07:42
tags:
  - AI
  - 工程化
readingTime: 8
description: "AI 已經能幫助完成跨檔案重構，但真正難的是控製風險。本文總結大型前端項目中使用 AI 做重構時的邊界劃分、測試護欄和審查方式。"
wordCount: 2239
---
...
這類重構需要**人類先劃定邊界**，AI 在每個邊界內執行。直接告訴 AI「把這個項目從 Redux 遷移到 Zustand」而不給出策略，很可能產生混亂。
...
**第一步：盤點現狀**
讓 AI 分析項目，列出所有 Vuex module、每個 module 的 state/getters/actions/mutations、模塊間的依賴關係、以及哪些組件引用了哪些模塊。
...
**第三步：逐個模塊遷移**
AI 逐個遷移 module，每個 module 完成後執行相關組件的測試。一個 module 驗證通過後再做下一個。

**第四步：清理和統一**
移除 Vuex 依賴，更新入口文件，統一命名和匯入路徑。
