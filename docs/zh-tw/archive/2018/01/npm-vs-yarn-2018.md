...
**1. 速度**：yarn 引入了本機快取，同一個套件第二次安裝幾乎是瞬間完成。npm 3 沒有可靠的快取機制，每次都要走網路。
...
團隊裡混用 npm 和 yarn 會導致 lockfile 衝突，強制統一工具版本：
...
**我的建議**：新專案用 npm 5+，除非有明確的 monorepo 需求。npm 是官方工具，跟隨 Node.js 發布，不需要額外安裝，維護成本更低。yarn 的主要優勢已經大幅縮小，只有 workspaces 是目前的明顯差異。
