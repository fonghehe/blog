...
鏈式呼叫中，任何一步丟擲錯誤，都會跳過後續 `then`，直接到 `catch`。
...
fetchUser(userId)
  .then((user) => {
    return fetchProfile(user.id); // 或者箭頭函式簡寫: user => fetchProfile(user.id)
  })
  .then((profile) => {
    console.log(profile);
  });
...
```javascript
// ❌ 這個 catch 捕獲不到 then 裡的錯誤
fetchUser()
  .catch((error) => console.error(error)) // catch 在前面，捕獲不到後面的錯誤
  .then((user) => {
    throw new Error("這裡的錯誤不會被捕獲");
  });

// ✅ catch 放在鏈的末尾
fetchUser()
  .then((user) => {
    throw new Error("這裡的錯誤會被捕獲");
  })
...
fetchUser().catch((error) => {
  setTimeout(() => {
    throw new Error("定時器裡的錯誤不在 Promise 鏈裡");
  }, 100);
});
...
## then 的第二個引數 vs catch

```javascript
// 方式一：then 的第二個引數（隻捕獲 fetchUser 的錯誤）
fetchUser().then(
...
async function loadUserPage(userId) {
  try {
    ...
    // fetchUser 或 fetchProfile 失敗都會到這裡
    handleError(error);
  }
}
```
