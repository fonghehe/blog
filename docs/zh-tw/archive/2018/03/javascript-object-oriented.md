---
title: "JavaScript 面向物件程式設計：原型鏈與類的設計"
...
JavaScript 裡每個物件都有一個隱式原型 `[[Prototype]]`，屬性查詢會沿著原型鏈向上找：
...
```javascript
class Counter {
  #count = 0; // 私有欄位（2018年已是 Stage 3 提案）
...
class Timer {
  ...
  start() {
    // ❌ setTimeout 裡的 this 不是 Timer 例項
