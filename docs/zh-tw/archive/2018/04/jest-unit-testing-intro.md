...
mockFn.mockReturnValue(42); // 設定返回值
mockFn.mockResolvedValue({ id: 1, name: "Alice" }); // 設定 Promise 返回值
...
不要追求 100%，工具函式、核心業務邏輯重點覆蓋，UI 展示類可以少測。

## 小結

- 先從純函式開始寫測試，門檻最低
- `expect().toBe()` 基礎，`toEqual()` 用於物件
- 非同步測試用 async/await
- 元件測試用 `@vue/test-utils`，測互動行為
- 有了測試，重構程式碼更有底氣
