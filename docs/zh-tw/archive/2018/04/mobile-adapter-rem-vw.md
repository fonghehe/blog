...
移動裝置螢幕寬度差異巨大：320px（iPhone 5）→ 414px（iPhone 8 Plus）→ 768px（iPad）。

設計師通常給 375px 的設計稿，需要在各種尺寸螢幕上等比縮放。
...
- `width=device-width`：視口寬度等於裝置寬度
- `initial-scale=1.0`：初始縮放比例 1:1
- `user-scalable=no`：禁止使用者縮放（爭議：無障礙訪問需要能縮放）

## 方案二：rem + flexible.js

rem 是相對於根元素 `<html>` 字型大小的單位。flexible.js（淘寶方案）動態設定根字型大小：
...
- iPhone 的 DPR = 2，意味著 1 CSS px = 2 物理畫素
- 1px 的邊框在 Retina 屏上看起來是 2 物理畫素寬
...
| 
--------------- | -------------------------------------- |
| rem + flexible  | 需要相容舊版 Android，團隊熟悉這套     |
| vw/vh           | 新專案，相容性要求 iOS 8+/Android 4.4+ |
| 固定寬度 + 居中 | PC 官網移動版，不需要等比縮放          |

2018 年的實際情況：主流方案還是 rem，vw/vh 在新專案裡可以用了。

## 小結

- rem 方案成熟，生態好，需要 JS 配合
- vw/vh 更簡潔，不依賴 JS，相容性已經夠用
- 兩者都配合 PostCSS 自動轉換，寫程式碼直接用設計稿 px 值
