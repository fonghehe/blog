...
tags:
  - CSS
readingTime: 1
description: "做了好幾個移動端專案，總結一下響應式設計的實踐經驗。"
---

做了好幾個移動端專案，總結一下響應式設計的實踐經驗。

## 移動優先（Mobile First）

先寫移動端樣式，然後用 `min-width` 媒體查詢逐步擴充套件到大屏：
...
```
1. Chrome DevTools：手機模擬器，選各種裝置尺寸
2. 實際裝置測試：尤其是 iOS Safari（和 Chrome 渲染差異大）
...
- 移動優先：從小屏開始寫，用 `min-width` 擴充套件
- 斷點選 576/768/992/1200（和主流框架對齊）
- Grid 的 `auto-fill + minmax` 比 Flex 更省媒體查詢
- 圖片用 `srcset`，字型用 `clamp()`
- 必須在真實裝置上測試 iOS Safari