...
tags:
  - Vue
readingTime: 1
description: "Vue 的響應式系統對物件屬性和陣列有一些特殊限製，搞清楚這些才能正確用 `watch`。"
wordCount: 226
---

Vue 的響應式系統對物件屬性和陣列有一些特殊限製，搞清楚這些才能正確用 `watch`。
...
**deep 的代價**：會遍歷物件所有屬性，效能開銷大。如果隻關心某個屬性，直接監聽那個路徑：
...
- Vue 2 陣列限製：索引賦值和 length 變化檢測不到，用 `$set` 或陣列方法
