---
title: "Vuex 模組化實踐：大型專案狀態管理"
date: 2018-06-19 14:45:49
tags:
  - Vue
readingTime: 2
description: "Vuex 入門容易，但專案規模大了之後，如何組織 store 是個挑戰。這篇文章分享大型專案的 Vuex 模組化方案。"
---

Vuex 入門容易，但專案規模大了之後，如何組織 store 是個挑戰。這篇文章分享大型專案的 Vuex 模組化方案。
...
const getters = {
  ...
  username: (state) => state.userInfo?.name || "未登入",
...
export default new Vuex.Store({
  ...
  // 嚴格模式：防止直接修改 state（生產環境不要開，有效能影響）
...
重新整理頁面後 Vuex state 重置，某些狀態需要持久化：
...
**不要在 mutation 裡做非同步操作：**

```javascript
// ❌ 錯誤
mutations: {
  async FETCH_USER(state) {
    state.user = await api.getUser()  // mutation 不能非同步！
  }
}
...
- 大型專案按業務模組拆分 store，開啟 `namespaced`
- mutations 只做同步狀態修改，非同步邏輯放 actions
- 合理使用持久化，不要把所有 state 都持久化
- `rootState` 和 `rootGetters` 實現跨模組訪問
