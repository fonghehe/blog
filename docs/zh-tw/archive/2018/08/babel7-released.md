---
title: "Babel 7 正式釋出：升級指南與新特性"
date: 2018-08-02 16:20:40
tags:
  - Babel
  - 工程化
readingTime: 2
description: "Babel 7 在 8 月正式釋出（經歷了超長的 beta 階段）。升級了幾個專案，整理一下主要變化和踩到的坑。"
---

Babel 7 在 8 月正式釋出（經歷了超長的 beta 階段）。升級了幾個專案，整理一下主要變化和踩到的坑。
...
module.exports = {
  ...
  plugins: [
    "@babel/plugin-transform-runtime",
    "@babel/plugin-proposal-optional-chaining", // ?. 運算子
    "@babel/plugin-proposal-nullish-coalescing-operator", // ?? 運算子
  ],
};
```
...
npm install @babel/runtime  # 執行時依賴（不是 devDependencies）
...
// Babel 會分析程式碼裡用了哪些新特性，自動引入相應 polyfill

// 例如：程式碼裡用了 Array.from
