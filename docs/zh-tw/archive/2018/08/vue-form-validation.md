    ...
    return {
      ...
      rules: {
        username: [
          ...
          { min: 2, max: 20, message: "長度 2-20 個字元", trigger: "blur" },
        ],
        email: [
...
- `validate` 驗證全部，`validateField` 驗證單個欄位
- 自定義驗證器：呼叫 `callback()` 表示通過，`callback(new Error())` 表示失敗
- 聯動驗證：watch 被依賴欄位，手動觸發 `validateField`
- 非同步驗證：自定義驗證器裡用 async/await
