...
Mock.js 可以生成隨機資料，攔截 XMLHttpRequest：
...
獨立的 REST API 服務，讀取 JSON 檔案，支援 CRUD：
...
優點：不需要修改業務程式碼，真實的網路請求，DevTools Network 可以看到。

## 對比

| 方案        | 難度 | 隨機資料 | 模擬延遲/錯誤 | 修改程式碼       |
| 
----------- | ---- | -------- | ------------- | -------------- |
| 硬編碼      | 極簡 | ❌       | ❌            | 是             |
| JSON 檔案   | 簡單 | ❌       | ❌            | 是             |
...
- 小專案：Mock.js + 隨機資料夠用
- 需要和後端資料結構對齊：JSON Server
- 單元/整合測試：MSW 是最佳實踐
- 無論哪種方案，Mock 程式碼不要進生產包（tree-shaking 或條件載入）
