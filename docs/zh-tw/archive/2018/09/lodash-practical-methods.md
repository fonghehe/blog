...
tags:
  - JavaScript
readingTime: 1
description: "Lodash 是前端專案裡最常用的工具庫之一，但很多人只用了一小部分。整理一下工作中最實用的方法。"
wordCount: 129
---

Lodash 是前端專案裡最常用的工具庫之一，但很多人只用了一小部分。整理一下工作中最實用的方法。
...
_.get(user, "address.country", "中國"); // '中國'（預設值）
...
_.sortBy(users, ["age", "name"]); // 多欄位排序
_.groupBy(users, "city"); // 按城市分組
_.keyBy(users, "id"); // 陣列轉以 id 為 key 的物件

// 實用場景：把使用者陣列轉成 id → user 的 Map
const userMap = _.keyBy(users, "id");
userMap["123"]; // 直接按 id 查詢
```
...
// 或者用 lodash-es（ES module 版本，支援 tree-shaking）
