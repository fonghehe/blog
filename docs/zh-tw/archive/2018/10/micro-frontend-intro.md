...
每個子應用可以有自己的技術棧、自己的釋出週期，團隊之間互不幹擾。
...
優點是隔離性極好——樣式、JS、全域性變數完全獨立。缺點也很明顯：路由同步困難，彈窗被 iframe 邊界截斷，頁面間通訊隻能靠 `postMessage`，而且 iframe 內的載入體驗比較差。
...
子應用需要暴露 `bootstrap`、`mount`、`unmount` 三個生命週期鉤子。思路很好，但目前社群還不大，檔案也比較簡陋，對 Vue 的支援需要額外適配。感覺是個有前途的方向，但現階段直接上生產環境風險不小。
...
iframe 內的 Modal 或 Toast 會被限製在 iframe 邊界內，無法覆蓋整個螢幕。這是 iframe 方案最頭疼的問題。我們的妥協是：彈窗在主應用中實現，子應用通過 `postMessage` 請求主應用彈窗。但這增加了耦合。
