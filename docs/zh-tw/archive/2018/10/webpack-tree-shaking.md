---
title: "Webpack Tree Shaking 原理和實踐"
date: 2018-10-06 10:55:15
tags:
  - Webpack
  - 工程化
readingTime: 1
description: "`tree-shaking` 這個詞來自 Rollup，意思是把沒用到的程式碼像樹葉一樣\"抖掉\"。Webpack 2 開始支援，但要正確配置才生效。"
wordCount: 226
---

`tree-shaking` 這個詞來自 Rollup，意思是把沒用到的程式碼像樹葉一樣"抖掉"。Webpack 2 開始支援，但要正確配置才生效。
...
// CommonJS：動態，無法分析
const math = require("./math"); // 整個物件都被引入，無法判斷用了哪些
const method = "add";
...
生產構建後，`multiply` 應該不在 bundle 裡。用 `webpack-bundle-analyzer` 驗證。
...
在我們專案裡，用了 lodash-es 並修復 sideEffects 配置之後：
...
- 配置 `sideEffects` 告知哪些檔案不可 tree-shake
