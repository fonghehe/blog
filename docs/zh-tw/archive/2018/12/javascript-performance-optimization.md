---
title: "前端效能最佳化：JS 執行效能"
date: 2018-12-15 10:33:56
tags:
  - 效能最佳化
readingTime: 2
description: "上個月排查了一個頁面卡頓問題，發現是 JS 執行時間過長導致掉幀。借這個機會整理一下 JS 效能最佳化的常用手段。"
wordCount: 180
---

上個月排查了一個頁面卡頓問題，發現是 JS 執行時間過長導致掉幀。借這個機會整理一下 JS 效能最佳化的常用手段。
...
如果 JS 執行超過 16ms，這一幀就會被推遲，使用者感覺到卡頓
```
...
self.addEventListener("message", (e) => {
  const { data } = e;
  const result = heavyCompute(data); // 在 worker 執行緒裡執行
