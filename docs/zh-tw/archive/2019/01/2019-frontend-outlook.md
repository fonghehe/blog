...
Figma 用 WebAssembly 路線實現了瀏核器端的高效能渲染。儘管大多數前端工程師不會編寫 Wasm，但瞭解它的使用場景（影片編製、CAD 渲染、加密庫）越來越重要。

## 工程化：目標是消滅設定

Create React App、Vue CLI 3、Angular CLI 共同指向同一個目標：讓開發者不再需要手動設定 webpack。優質的框架封裝 = 更快的專案啟動速度 + 更一致的團隊設定。
...
| TypeScript | 在新專案中強製開啟。學習 Generic、Mapped Types |
