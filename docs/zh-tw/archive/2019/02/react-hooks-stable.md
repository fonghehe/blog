...
function Counter() {
  ...
  }, [count]); // 依賴陣列：隻有 count 變化時才重新執行

  // 第二引數為 []：隻在掛載時執行一次（componentDidMount）
  useEffect(() => {
    console.log("元件掛載");
...
1. **隻在最頂層呼叫 Hook**：不能在迴圈、條件、巢狀函式中呼叫
2. **隻在 React 函式中呼叫 Hook**：函式元件 或 自定義 Hook
