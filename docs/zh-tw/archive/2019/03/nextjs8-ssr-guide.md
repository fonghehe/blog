---
title: "Next.js 8 深度上手：SSR 與靜態匯出"
...
- React 官方推薦的服務端渲染框架
- 檔案系統路由，不用配置 react-router
- 支援 SSR、SSG（靜態生成）、混合模式
- 8.0 版本加入了自動靜態最佳化（Automatic Static Optimization）
...
```jsx
// pages/_app.js：全域性 layout、全域性樣式、路由變化
...
```jsx
// pages/_document.js：自定義 HTML 結構（加第三方指令碼、meta 標籤）
...
import Image from "next/image"; // 自動圖片最佳化（Next.js 10+ 特性，先記錄）
...
- Next.js 檔案即路由，SSR/SSG 混合模式
- `getInitialProps` 在服務端和客戶端都可能執行（注意區分環境）
- API Routes 讓 Next.js 可以做輕量全棧
- 沒有 `getInitialProps` 的頁面會自動靜態化，效能最好
- 專案實踐建議：官網類用靜態匯出，後臺管理用 CSR，電商用 SSR
