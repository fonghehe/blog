...
不配這個，vue-router history 模式重新整理會 404。
...
Webpack 構建時可以預先生成 `.gz` 檔案（`compression-webpack-plugin`），Nginx 直接傳送：

```nginx
location ~* \.(js|css|html)$ {
  gzip_static on;  # 優先發送 .gz 檔案
}
```
