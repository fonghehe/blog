...
tags:
  - 效能最佳化
readingTime: 2
description: "專案上線後出了問題，往往是使用者先發現，然後反饋給運營，再到研發。建立一套監控體系，讓我們比使用者先發現問題。"
---

專案上線後出了問題，往往是使用者先發現，然後反饋給運營，再到研發。建立一套監控體系，讓我們比使用者先發現問題。
...
| 自建（上報+ELK） | 資料自控、成本可控       | 運維成本高         |

中小團隊推薦 Sentry，資料量大的大團隊考慮自建。

## 小結

- 錯誤監控：`window.error`、`unhandledrejection`、框架錯誤處理
- 效能監控：Performance API、Paint Timing API
- `sendBeacon` 保證頁面解除安裝時也能傳送資料
- 生產 Source Map 上傳到監控平臺，還原可讀的錯誤堆疊
