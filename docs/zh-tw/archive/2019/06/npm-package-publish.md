...
- `npm unpublish`：從 registry 徹底刪除，其他依賴它的專案會安裝失敗。npm 有 72 小時限製，且如果有人依賴了你的包則不允許 unpublish
- `npm deprecate`：包還在，隻是安裝時會顯示警告。對已有使用者更友好
...
- 用 `files` 欄位白名單控製釋出內容，比 `.npmignore` 更安全
