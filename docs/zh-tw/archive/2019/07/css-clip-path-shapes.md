...
`clip-path` 允許你定義一個裁切區域，隻顯示區域內的內容。它支援五種基本形狀函式：
...
`clip-path` 支援 CSS transition 和 animation，前提是**形狀函式相同且頂點數量一致**：
...
**踩過的坑：** 從 `circle()` 過渡到 `polygon()` 是不行的，因為兩種形狀函式無法插值。必須保持函式型別一致：
...
- `clip-path` 支援 `inset()`、`circle()`、`ellipse()`、`polygon()`、`url()` 五種形狀函式
- `polygon()` 最強大，可以定義任意多邊形，用百分比或長度值指定頂點
- 支援 CSS transition 動畫，但要求形狀函式型別相同、頂點數量一致
- 配合 `shape-outside` 可以實現文字環繞不規則形狀
- Safari 需要 `-webkit-` 字首，IE 完全不支援，需要降級方案
