...
tags:
  ...
  - 工程化
readingTime: 2
description: "公司的老後臺系統是 2016 年的 jQuery 專案，新功能想用 Vue，但不能全量重寫。微前端解決了這個問題。"
wordCount: 430
---

公司的老後臺系統是 2016 年的 jQuery 專案，新功能想用 Vue，但不能全量重寫。微前端解決了這個問題。

## 我們的場景

- 老系統：jQuery + Bootstrap，幾十個頁面
- 新需求：新模組要用 Vue CLI 3 開發
- 目標：新老共存，逐步遷移，統一導航
...
registerApplication(
  "legacy",
  () => import("./apps/legacy-app"), // 載入函式
  (location) => location.pathname.startsWith("/legacy"), // 啟用條件
);
...
// 方案 3：Shadow DOM（侵入性最低，但 IE 不支援）
...
```javascript
// shell/src/eventBus.js（全域性事件匯流排）
class EventBus {
  constructor() {
    this.events = {};
  }
  ...
  on(event, callback) {
    ...
    return () => this.off(event, callback); // 返回取消訂閱函式
  }
...
1. **全域性變數汙染**：子應用解除安裝時要清理註冊的全域性變數、事件監聽
2. **CSS 全域性汙染**：第三方 UI 庫（Element UI）的全域性樣式會互相影響
3. **路由衝突**：子應用路由要用 `base` 配置，和主應用路由不衝突
4. **依賴重複**：每個子應用都打包 Vue，體積大。可以用 CDN 共享

## 我們的遷移路線

第一階段（3 個月）：新功能模組用 Vue 開發，通過微前端掛載
...
- 樣式隔離推薦 CSS Modules，子應用通訊用全域性事件匯流排
- 常見坑：全域性變數汙染、CSS 汙染、路由衝突
