...
tags:
  - 測試
readingTime: 4
description: "前端自動化測試一直是團隊落地的難點。單元測試覆蓋不到使用者真實操作流程，Selenium 又配置複雜、執行不穩定。Cypress 是一個專門為現代前端應用設計的 E2E 測試框架，開箱即用、API 簡潔、自帶自動重試機制，是目前社群中非常受歡迎的選擇。本文將從零搭建 Cypress 測試環境，逐步覆蓋核心功能。"
---

前端自動化測試一直是團隊落地的難點。單元測試覆蓋不到使用者真實操作流程，Selenium 又配置複雜、執行不穩定。Cypress 是一個專門為現代前端應用設計的 E2E 測試框架，開箱即用、API 簡潔、自帶自動重試機制，是目前社群中非常受歡迎的選擇。本文將從零搭建 Cypress 測試環境，逐步覆蓋核心功能。
...
- 自動重試機制讓測試更穩定，不需要手動新增 `sleep` 或 `wait`
