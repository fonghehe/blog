---
title: "Cypress E2E 測試入門與實踐"
date: 2019-08-09 09:44:05
tags:
  - 測試
readingTime: 4
description: "前端自動化測試一直是團隊落地的難點。單元測試覆蓋不到使用者真實操作流程，Selenium 又設定複雜、執行不穩定。Cypress 是一個專門為現代前端應用設計的 E2E 測試框架，開箱即用、API 簡潔、自帶自動重試機製，是目前社群中非常受歡迎的選擇。本文將從零搭建 Cypress 測試環境，逐步覆蓋核心功能。"
wordCount: 526
---

前端自動化測試一直是團隊落地的難點。單元測試覆蓋不到使用者真實操作流程，Selenium 又設定複雜、執行不穩定。Cypress 是一個專門為現代前端應用設計的 E2E 測試框架，開箱即用、API 簡潔、自帶自動重試機製，是目前社群中非常受歡迎的選擇。本文將從零搭建 Cypress 測試環境，逐步覆蓋核心功能。
...
首次執行 `npx cypress open` 會自動建立以下目錄結構：
...
建立 `cypress/integration/home.spec.js`：
...
Cypress 內建了 Chai 斷言庫，支援 BDD 風格的 `should` 和 `expect`：
...
describe('使用者列表', () => {
  ...
  it('應該處理請求失敗', () => {
    ...
    cy.route({
      ...
      url: '/api/users',
      status: 500,
      response: { error: '伺服器錯誤' },
    }).as('getUsersError');

    cy.visit('/users');
    cy.wait('@getUsersError');

    cy.get('.error-message').should('contain', '伺服器錯誤');
  });
});
```
...
建立 `cypress/fixtures/users.json`：
...
- 自動重試機製讓測試更穩定，不需要手動新增 `sleep` 或 `wait`
- 推薦使用 `data-testid` 作為選擇器，避免測試因樣式變化而失敗
- 使用 fixtures 模擬 API 資料，讓測試不依賴後端環境
- 自定義命令和頁面物件模式可以有效組織測試程式碼，減少重複
- CI 整合時使用 `cypress run`（無頭模式），配合 `wait-on` 確保應用就緒後再執行測試
- 注意合理設定超時時間和 viewport 大小，適應不同場景
