...
tags:
  - CSS
readingTime: 3
description: "在響應式設計中，我們一直依賴 `@media` 查詢根據視口（viewport）大小來調整佈局。但這種方式有一個根本性的缺陷：元件的行為取決於視口，而不是元件自身的尺寸。Container Queries 提案試圖解決這個問題，讓元件可以根據其父容器的尺寸來調整自身樣式。"
---

在響應式設計中，我們一直依賴 `@media` 查詢根據視口（viewport）大小來調整佈局。但這種方式有一個根本性的缺陷：元件的行為取決於視口，而不是元件自身的尺寸。Container Queries 提案試圖解決這個問題，讓元件可以根據其父容器的尺寸來調整自身樣式。
...
問題在於：即使視口很寬，如果卡片被放在窄的側邊欄裡，它仍然應該以窄屏模式展示。`@media` 查詢無法處理這種場景。
...
在原生支援到來之前，可以用 JavaScript + ResizeObserver 模擬：
...
- **@media**：適用於全域性佈局決策（頁面級響應式）
- **@container**：適用於元件級響應式（元件自適應）
...
- Container Queries 讓元件可以根據父容器尺寸調整樣式
- 提案核心語法：`container-type`、`container-name`、`@container`
- 解決了元件庫在不同佈局上下文中自適應的痛點
- 截至 2019 年仍處於提案階段，尚無瀏覽器原生支援
- 可用 ResizeObserver + CSS 類名方式模擬實現
- React Hook 封裝可以簡化元件中的使用
