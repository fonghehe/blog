...
tags:
  - JavaScript
readingTime: 3
description: "可選鏈運算子（Optional Chaining）`?.` 是 TC39 Stage 3 提案，預計將成為 ES2020 標準的一部分。它允許安全地訪問深層巢狀的物件屬性，而不需要在每一層都做空值檢查。"
wordCount: 425
---

可選鏈運算子（Optional Chaining）`?.` 是 TC39 Stage 3 提案，預計將成為 ES2020 標準的一部分。它允許安全地訪問深層巢狀的物件屬性，而不需要在每一層都做空值檢查。

## 問題背景

在日常開發中，我們經常需要訪問深層巢狀的物件屬性：
...
processData(input, console.log);  // 傳回調，正常呼叫
```
...
// 組合使用
const displayName = user?.profile?.nickname ?? '匿名使用者';
// '小明'

const email = user?.profile?.email ?? '未設定';
// '未設定'
...
可以看到編譯後的程式碼確實比手寫 `&&` 要長一些，但可讀性在原始碼層面大大提升。

## 小結

- 可選鏈 `?.` 安全地訪問深層巢狀屬性，中間層為 null/undefined 時返回 undefined
- 支援三種用法：屬性訪問 `?.prop`、陣列索引 `?.[index]`、函式呼叫 `?.()`
- 與空值合併運算子 `??` 配合使用可以提供預設值
- TypeScript 3.7+ 原生支援，Babel 也有外掛支援
- 不能用於賦值操作左側
