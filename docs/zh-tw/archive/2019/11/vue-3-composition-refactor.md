...
tags:
  - Vue
readingTime: 5
description: "Vue 3 的 Composition API 引入了一種全新的元件邏輯組織方式。本文通過一個真實的使用者管理頁面，演示如何用 Composition API 重構 Vue 2 的 Options API 程式碼，展示 Composable 函式的抽取和複用。"
wordCount: 420
---

Vue 3 的 Composition API 引入了一種全新的元件邏輯組織方式。本文通過一個真實的使用者管理頁面，演示如何用 Composition API 重構 Vue 2 的 Options API 程式碼，展示 Composable 函式的抽取和複用。

## 重構前：Options API

假設有一個使用者管理頁面，包含搜尋、列表展示、分頁、編輯彈窗等功能：
  ...
  <div class="user-management">
    <div class="toolbar">
      <input
        ...
        @input="handleSearch"
      />
      <button @click="showCreateDialog = true">新建使用者</button>
    </div>
...
問題：相關邏輯分散在 `data`、`computed`、`watch`、`methods` 中，隨著功能增加，檔案會變得很長且難以維護。
...
├── useSearch.js        // 搜尋邏輯
├── useCrud.js          // CRUD 操作
├── useConfirm.js       // 確認對話方塊
├── useMousePosition.js // 滑鼠位置
├── useDebounce.js      // 防抖
├── useThrottle.js      // 節流
└── useFetch.js         // 資料獲取
```
...
|
------|-----------------|------------|
| 響應性 | 自動追蹤依賴 | 需要手動指定依賴陣列 |
| 呼叫順序 | 無限製 | 必須在頂層呼叫 |
| 副作用清理 | onUnmounted | useEffect 返回清理函式 |
| 複用方式 | 函式呼叫 | 函式呼叫 |
| 狀態隔離 | 每次呼叫建立獨立狀態 | 每次呼叫建立獨立狀態 |

## 小結

- Composition API 通過 `setup()` 函式替代 Options API 的分散寫法
- Composable 函式可以將相關邏輯抽取為可複用的模組
- `usePagination`、`useSearch`、`useCrud` 等常見的 Composable 可以跨元件複用
- 命名約定以 `use` 開頭，與 React Hooks 一致
- Composition API 不替代 Options API，兩者可以共存
- 適合在大型專案中組織複雜邏輯，小型專案用 Options API 也完全足夠
