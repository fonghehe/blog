---
title: "CSS Grid template-areas 佈局實戰"
date: 2019-12-05 10:47:52
tags:
  - CSS
readingTime: 3
description: "CSS Grid 佈局規範已經出來幾年了，瀏覽器支援也相當完善（IE11 部分支援帶字首）。但實際專案中，很多同學還是習慣用 Flexbox 解決所有佈局問題。最近在重構後臺管理系統的頁面佈局時，我全面採用了 Grid 的 `template-areas` 方案，效果出乎意料的好。這篇文章分享一些實戰經驗。"
wordCount: 534
---

CSS Grid 佈局規範已經出來幾年了，瀏覽器支援也相當完善（IE11 部分支援帶字首）。但實際專案中，很多同學還是習慣用 Flexbox 解決所有佈局問題。最近在重構後臺管理系統的頁面佈局時，我全面採用了 Grid 的 `template-areas` 方案，效果出乎意料的好。這篇文章分享一些實戰經驗。
...
<div class="dashboard">
  <div class="stats-row">
    <div class="stat-card">日活使用者: 12,345</div>
...
`template-areas` 最大的優勢之一是響應式佈局極其直觀——只需要重新"畫"區域圖即可，不需要改 HTML 結構：
...
Grid 負責宏觀頁面佈局，Flexbox 負責元件內部排列，兩者配合是最佳實踐：
...
- 響應式佈局只需在斷點處重新定義 `template-areas`，無需改動 HTML 結構
- Grid 負責宏觀佈局，Flexbox 負責元件內部排列，兩者配合是最佳實踐
- 區域名稱必須形成矩形，用 `.` 表示空白區域
- 配合 `minmax`、`auto-fit` 和 CSS 變數，可以構建靈活且可維護的佈局系統
- 瀏覽器相容性已經非常好，IE11 帶 `-ms-` 字首部分支援，現代瀏覽器全面支援
