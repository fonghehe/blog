...
Nullish Coalescing 運算子 `??` 與 `||` 類似，但隻在左側為 `null` 或 `undefined` 時才返回右側值。這個區別在處理 `0`、`''`、`false` 等 falsy 值時非常重要：
...
BigInt 是 JavaScript 新增的基本資料型別，用於表示任意精度的整數。它解決了 `Number.MAX_SAFE_INTEGER` (2^53 - 1) 的限製：
