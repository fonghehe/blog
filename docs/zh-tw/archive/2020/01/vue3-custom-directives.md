...
tags:
  - Vue
readingTime: 2
description: "自定義指令在 Vue 3 中經過了重設計，生命週期鉤子和 Composition API 保持了一致的命名風格。對於需要直接操作 DOM 的場景——拖拽、防抖點選、許可權控製——自定義指令仍然是最乾淨的抽象方式。"
wordCount: 343
---

自定義指令在 Vue 3 中經過了重設計，生命週期鉤子和 Composition API 保持了一致的命名風格。對於需要直接操作 DOM 的場景——拖拽、防抖點選、許可權控製——自定義指令仍然是最乾淨的抽象方式。
