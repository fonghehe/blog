...
tags:
  - Vue
readingTime: 3
description: "Vue 3 要求顯式宣告元件發出的事件，這不僅是最佳實踐，更是一種強製約束。配合 TypeScript 的型別推導，可以在編譯期就捕獲大部分事件相關的 bug。本文從 emit 的底層機製講起，整理事件設計的最佳實踐。"
wordCount: 370
---

Vue 3 要求顯式宣告元件發出的事件，這不僅是最佳實踐，更是一種強製約束。配合 TypeScript 的型別推導，可以在編譯期就捕獲大部分事件相關的 bug。本文從 emit 的底層機製講起，整理事件設計的最佳實踐。
...
當驗證函式返回 `false` 時，控製臺會列印警告資訊，但不會阻止事件觸發。這適合開發環境排查問題。
...
- 使用物件語法宣告 emits 可以做引數型別校驗，返回 false 會觸發控製臺警告
