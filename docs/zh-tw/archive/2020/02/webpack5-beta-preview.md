...
這是 Webpack 5 最重磅的特性，允許多個獨立構建的應用共享模組：
...
這意味著微前端不再需要 iframe 或 single-spa 那麼重的方案了。

## 持久化快取

Webpack 5 內建了持久化快取，不再需要 `hard-source-webpack-plugin`：
...
內建了資源處理，不再需要 `file-loader`、`url-loader`、`raw-loader`：
...
// Webpack 5 新增：
// 1. 巢狀 tree shaking：連巢狀模組的匯出也能分析
// 2. 內部模組 tree shaking：模組內部的匯出也能被 shake
