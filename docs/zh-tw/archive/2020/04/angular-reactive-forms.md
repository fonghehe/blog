...
tags:
  - React
readingTime: 2
description: "Angular 提供了兩種表單方案：Template-driven 和 Reactive Forms。對於需要動態欄位、跨欄位驗證、程式化控製的複雜場景，Reactive Forms 是唯一合適的選擇。"
wordCount: 152
---

Angular 提供了兩種表單方案：Template-driven 和 Reactive Forms。對於需要動態欄位、跨欄位驗證、程式化控製的複雜場景，Reactive Forms 是唯一合適的選擇。
...
@Component({
  template: `
    ...
    <button type="button" (click)="addContact()">新增聯絡方式</button>
  `,
})
...
this.fb.control("", {
  ...
  updateOn: "blur", // 失焦時才觸發驗證，減少 API 呼叫
});
```
...
Angular Reactive Forms 的強大之處在於**表單邏輯完全在 TypeScript 中**，便於測試和複用。核心三件套：`FormGroup`（欄位組）、`FormArray`（動態列表）、非同步驗證器，掌握這三個就能應對 90% 的複雜表單場景。
