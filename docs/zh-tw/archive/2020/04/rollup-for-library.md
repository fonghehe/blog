...
tags:
  - 工程化
readingTime: 2
description: "用 Webpack 打包應用，但打包庫的時候 Rollup 更合適。為什麼？因為 Rollup 產物更乾淨、更小。記錄一下用 Rollup 打包一個 Vue 元件庫的全過程。"
wordCount: 187
---

用 Webpack 打包應用，但打包庫的時候 Rollup 更合適。為什麼？因為 Rollup 產物更乾淨、更小。記錄一下用 Rollup 打包一個 Vue 元件庫的全過程。
...
| 產物體積 | 較大（執行時多）   | 較小（無執行時）   |
| Tree Shaking | 支援          | 更徹底           |
| Code Splitting | 支援         | 有限             |
```

簡單說：Webpack 管理應用，Rollup 打包庫。
...
- `peerDependencies` 宣告 Vue 版本，不把 Vue 打包進去
- `sideEffects` 宣告副作用檔案，讓使用者能按需引入
- 樣式建議提取為獨立 CSS 檔案，而不是內聯到 JS
