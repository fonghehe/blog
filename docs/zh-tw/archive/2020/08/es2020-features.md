...
ES2020（ES11）規範已經正式釋出，帶來了幾個非常實用的特性。在日常程式碼中用起來，能明顯減少模板程式碼。
...
// 轉換
BigInt('12345678901234567890'); // 從字串
...
- 可選鏈和空值合併是最實用的特性，日常程式碼每天都會用到
- BigInt 處理大整數，適合資料庫 ID、金融計算等場景
- Promise.allSettled 解決批次請求中部分失敗的需求
- globalThis 統一了全域性物件的訪問方式
- 這些特性在現代瀏覽器中都已支援，老專案可用 Babel 降級
