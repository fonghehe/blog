---
title: "Node.js 14 新特性與 Optional Chaining 支援"
date: 2020-08-17 10:39:36
tags:
  - JavaScript
readingTime: 3
description: "Node.js 14 LTS 在 4 月份釋出，帶來了不少實用特性。最讓前端工程師開心的是 V8 引擎升級到了 8.1，原生支援了 Optional Chaining（`?.`）和 Nullish Coalescing（`??`）—— 終於不用再在 Node 程式碼裡手動轉譯這些語法了。"
---

Node.js 14 LTS 在 4 月份釋出，帶來了不少實用特性。最讓前端工程師開心的是 V8 引擎升級到了 8.1，原生支援了 Optional Chaining（`?.`）和 Nullish Coalescing（`??`）—— 終於不用再在 Node 程式碼裡手動轉譯這些語法了。
...
const timeout = options.timeout ?? 30000; // 0 表示不超時，undefined 預設 30 秒

// 和可選鏈組合使用
const name =
  user?.profile?.displayName ?? user?.profile?.username ?? "匿名使用者";
```

## Array.flat() 和 Array.flatMap()

Node.js 14 的 V8 8.1 也原生支援了這些方法：
...
const results = ids.flatMap((id) => {
  ...
  return item ? [item] : []; // 沒找到就返回空陣列，相當於過濾
});
```
...
- tsconfig 的 target 可以設定為 ES2020 來利用這些特性
