...
先看當前 Composition API 的寫法，一個簡單的元件需要大量樣板程式碼：
...
每個變數都要在 `return` 裡宣告一次，元件越大樣板越多。`<script setup>` 直接解決了這個問題：
...
`import` 進來的元件自動註冊，不需要在 `components` 裡手動宣告：
...
預設情況下，`<script setup>` 暴露的是頂層繫結。如果需要暴露子元件的特定方法：
...
`<script setup>` 對 TypeScript 的支援很自然：
...
function handleChange(value: string) {
  emit('change', value) // 型別檢查：value 必須是 string
}
</script>
```
...
程式碼量減少了，邏輯也更聚合——相關的狀態和邏輯在一起，不用在 `data`、`computed`、`methods` 之間跳來跳去。

## 小結

- `<script setup>` 是 Composition API 的語法糖，大幅減少樣板程式碼
- 頂層繫結自動暴露，元件 import 自動註冊，開發體驗接近 React Hooks
- `defineProps` / `defineEmits` / `defineExpose` 是編譯宏，不需要 import
- TypeScript 支援自然，型別推斷比 Options API 好很多
- 預計 Vue 3.2 正式支援，現在可以先在實驗性專案中試用
