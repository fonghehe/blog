...
`useTransition` 是 React 18 的核心新 API，用於標記低優先順序的狀態更新：
...
核心概念：`startTransition` 內的狀態更新會被標記為"可以被打斷"。如果使用者快速輸入，React 會丟棄舊的渲染，優先響應新的輸入——不會出現輸入卡頓的情況。
...
注意：用了新的 `createRoot` API 後，自動批處理就生效了。不改 API 的話預設還是 React 17 行為，可以漸進升級。
...
- React 18 自動批處理無需改程式碼，升級後所有場景預設合併更新
- `useTransition` 解決了輸入卡頓等效能問題，核心思路是區分優先順序
- `useDeferredValue` 適合"顯示舊資料比顯示空白好"的場景
