...
tags:
  ...
  - TypeScript
readingTime: 2
description: "團隊計劃從 Element UI 遷移到自建 Vue 3 元件庫。用 Vite 做構建工具比 Rollup 或 Webpack 更簡潔，特別是處理 Vue SFC 時。記錄一下搭建過程。"
wordCount: 193
---

團隊計劃從 Element UI 遷移到自建 Vue 3 元件庫。用 Vite 做構建工具比 Rollup 或 Webpack 更簡潔，特別是處理 Vue SFC 時。記錄一下搭建過程。
...
- Vite 的 library mode 適合構建 Vue 3 元件庫，配置比 Rollup 簡潔
- 用 `vite-plugin-dts` 自動生成型別宣告，按需引入體驗更好
- `exports` 欄位配合 `sideEffects` 確保 tree-shaking 生效
- Vue SFC 的 `<script setup>` 寫元件很自然，型別支援也好
- 建議用 pnpm workspace 管理元件庫 + 示例應用 + 文件站的 Monorepo