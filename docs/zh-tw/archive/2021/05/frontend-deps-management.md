...
今年處理了好幾次因為依賴問題導致的線上事故：版本升級導致的相容性問題、lockfile 衝突導致的構建差異、依賴安全漏洞。總結一下在團隊中管理前端依賴的經驗。
...
// package.json 中版本號的含義
{
  "dependencies": {
    ...
    // ^1.2.3 允許 1.x.x 的最新 minor（npm/yarn 預設）
    // 1.2.3  精確鎖定
