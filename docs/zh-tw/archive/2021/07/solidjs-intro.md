...
tags:
  ...
  - JavaScript
readingTime: 2
description: "SolidJS 是 2021 年最值得關注的前端框架之一。它的核心主張聽起來和 Svelte 相似——\"不用虛擬 DOM\"——但實現原理完全不同。Svelte 是編譯時框架，SolidJS 是執行時細粒度響應式。這篇文章帶你瞭解它為什麼能在效能測試中長期排名第一。"
---

SolidJS 是 2021 年最值得關注的前端框架之一。它的核心主張聽起來和 Svelte 相似——"不用虛擬 DOM"——但實現原理完全不同。Svelte 是編譯時框架，SolidJS 是執行時細粒度響應式。這篇文章帶你瞭解它為什麼能在效能測試中長期排名第一。
...
- React：狀態變化 → 元件重新渲染 → Virtual DOM diff → DOM 更新
...
注意：`count` 是函式，讀取時要呼叫 `count()`——這是 SolidJS 和 React 最明顯的語法區別。
...
JS 框架效能測試（js-framework-benchmark）中，SolidJS 始終排名前列，接近原生 JavaScript：

| 框架     | 建立1000行 | 替換1000行 | 部分更新 |
| -------- | ---------- | ---------- | -------- |
...
SolidJS 證明了"React 式 API + 細粒度響應式"是可行的。它不一定會取代 React 或 Vue，但它的設計思想正在影響整個生態——Vue 3 的 `@vue/reactivity` 和 Preact Signals 都有 SolidJS 的影子。2021 年值得用它做個小專案體驗一下。