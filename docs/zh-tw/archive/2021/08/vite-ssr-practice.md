...
最近把一個內部運營平臺從 CSR 遷移到了 Vite SSR，記錄一下踩坑和經驗。Vite 2.x 的 SSR 支援已經比較成熟了，但文件偏少，實際用起來還是有一些要注意的。
...
// hydrate：把服務端渲染的 HTML "啟用"為可互動的 SPA
router.isReady().then(() => {
...
export function useAsyncData<T>(key: string, fetcher: () => Promise<T>) {
  ...
  // 服務端：直接呼叫
  // 客戶端：檢查 window.__SSR_DATA__
  if (import.meta.env.SSR) {
    // 服務端執行，結果通過 HTML 注入
  } else {
    // 客戶端 hydrate 時從 __SSR_DATA__ 讀取
...
- Vite SSR 開發體驗很好（HMR、即時編譯），但生產部署要注意 CSS 和靜態資源處理
- 瀏覽器 API 的相容是最大的坑，`import.meta.env.SSR` 是核心判斷依據
- 資料預取需要額外設計（不像 Next.js 有完整的約定）
- 適合對首屏速度有要求的 Vue 3 專案；SEO 場景也可以用，但維護成本比 Nuxt 高