...
這是 Next.js 12 最大的新特性——在請求到達頁面之前執行程式碼：
...
Middleware 執行在 Edge Runtime 上，比 Node.js 更輕量，響應更快。適合做認證、重定向、A/B 測試這類輕量邏輯。
...
Next.js 12 開始支援 React 18 的部分特性：
...
可以直接從 URL 匯入模組，不需要 npm 安裝：

```javascript
// 實驗性特性，需要在 next.config.js 中啟用
// next.config.js
module.exports = {
  experimental: {
...
- Middleware 是 Next.js 12 最重要的新特性，執行在 Edge Runtime
- React 18 的 Suspense SSR 支援讓頁面載入體驗更好
