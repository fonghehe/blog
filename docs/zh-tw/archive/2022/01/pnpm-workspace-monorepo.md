...
tags:
  - 前端工程化
readingTime: 2
description: "團隊把多個前端專案遷移到 pnpm workspace 已經半年了。磁碟佔用降了 60%，安裝速度提升明顯，幽靈依賴問題徹底解決。這篇文章記錄我們的實踐過程。"
wordCount: 435
---

團隊把多個前端專案遷移到 pnpm workspace 已經半年了。磁碟佔用降了 60%，安裝速度提升明顯，幽靈依賴問題徹底解決。這篇文章記錄我們的實踐過程。

## 為什麼選 pnpm

npm 和 yarn 的 node_modules 是扁平結構，依賴提升帶來兩個問題：

1. **幽靈依賴**：你可以在程式碼裡 import 沒宣告的包（因為被提升到了頂層）
2. **磁碟浪費**：不同專案各自維護一份完整的 node_modules

pnpm 用硬連結 + 內容定址儲存解決這兩個問題：
...
`node-linker` 有三個選項：
- `isolated`（預設）：嚴格的符號連結結構，相容性最差但最安全
- `hoisted`：類似 npm/yarn 的扁平結構，相容性最好
- `hoisted`：折中方案

我們選 `hoisted` 是因為有些老依賴不支援嚴格的 node_modules 結構。
...
```json
"ui-components": "workspace:^"   // 釋出時替換為 ^1.2.3
"ui-components": "workspace:*"   // 釋出時替換為 1.2.3（精確版本）
"ui-components": "workspace:~"   // 釋出時替換為 ~1.2.3
```
...
# 5. 用 pnpm filter 替換原來的指令碼
```

## 小結

pnpm workspace 在磁碟效率和依賴隔離上做到了最佳平衡。對於前端 monorepo，它是目前最務實的選擇。接下來我會寫 Turborepo 來做構建編排，配合 pnpm 組成完整的 monorepo 工具鏈。