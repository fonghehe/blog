...
tags:
  - Angular
readingTime: 2
description: "Angular Router 是 Angular 生態中最成熟的路由方案之一。這篇文章深入講解懶載入設定、路由守衛的各種形式以及大型應用的路由組織模式，適合已經熟悉基礎 Angular 路由的開發者。"
wordCount: 243
---
...
- `canLoad`：阻止懶載入模組的程式碼**下載**，更徹底的許可權控製
...
// 自定義預載入策略：隻預載入標記了 data.preload: true 的模組
@Injectable({ providedIn: 'root' })
...
Angular Router 的功能遠不止基礎路由設定。`canLoad` 在許可權控製上比 `canActivate` 更徹底；`Resolve` 守衛確保元件渲染時資料已就緒；自定義預載入策略能精確控製模組下載時機。合理使用這些特性，可以讓大型 Angular 應用的路由層既安全又高效。