...
以前用媒體查詢，隻能根據瀏覽器視口來決定樣式——但這裡的卡片元件需要根據父容器寬度來決定佈局。
...
Chrome 105+、Edge 105+、Safari 16+、Firefox 110+。對於不支援的瀏覽器，可以用 PostCSS 外掛降級：
...
Container Queries 解決了元件化開發中最基本的需求：讓元件根據自己的容器響應式調整佈局，而不是依賴全域性視口。這是 CSS 響應式設計從「頁面級」到「元件級」的進化。現在主流瀏覽器都已支援，是時候在元件庫中用起來了。