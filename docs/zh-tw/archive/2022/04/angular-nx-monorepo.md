...
tags:
  ...
  - 前端工程化
readingTime: 2
description: "Nx 是專為 Monorepo 設計的構建系統，對 Angular 有一流支援。它解決了大型前端專案的兩個核心問題：**程式碼共享**（多個應用共用同一套元件/工具庫）和**構建速度**（隻重新構建受影響的部分）。這篇文章介紹 Nx + Angular 的實踐方案。"
wordCount: 345
---

Nx 是專為 Monorepo 設計的構建系統，對 Angular 有一流支援。它解決了大型前端專案的兩個核心問題：**程式碼共享**（多個應用共用同一套元件/工具庫）和**構建速度**（隻重新構建受影響的部分）。這篇文章介紹 Nx + Angular 的實踐方案。
...
## Affected 命令：隻構建受影響的專案

```bash
# 隻測試受當前變更影響的專案（對比 main 分支）
...
Nx + Angular 的組合在大型專案中優勢明顯：`affected` 命令讓 CI 時間隨專案增長保持線性而非指數級增長；共享庫通過 TypeScript path alias 實現零設定複用；模組邊界約束防止迴圈依賴和架構腐化。如果你的團隊維護 2 個以上 Angular 應用並有共享程式碼的需求，Nx Monorepo 是值得投入的選項。