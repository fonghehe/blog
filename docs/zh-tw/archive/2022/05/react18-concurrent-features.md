...
tags:
  - React
readingTime: 3
description: "React 18 正式版釋出兩個月了，併發特性不再是實驗室裡的概念。這篇深入兩個核心 API——`useTransition` 和 `useDeferredValue`，用真實場景講解怎麼在專案中落地。"
---

React 18 正式版釋出兩個月了，併發特性不再是實驗室裡的概念。這篇深入兩個核心 API——`useTransition` 和 `useDeferredValue`，用真實場景講解怎麼在專案中落地。

## useTransition：標記低優先順序更新

核心思想：不是所有狀態更新都一樣緊急。使用者輸入是緊急的，搜尋結果更新是不緊急的。
...
`isPending` 告訴你 transition 還在進行中——可以用來顯示載入狀態，但輸入框不會卡頓。
...
點 Tab 時，按鈕立即高亮（緊急更新），內容區域可以稍後更新（低優先順序）。如果沒有 transition，500ms 的渲染會讓 Tab 點選延遲響應。
...
`isStale` 的判斷技巧：當原始值和延遲值不一致，說明延遲更新還在進行中，可以用視覺降級（降低透明度）提示使用者。
...
function A() {
  ...
  function handleClick() {
    startTransition(() => {
      setState(newValue); // 你決定哪些 setState 是低優先順序
    });
  }
}
...
useTransition 和 useDeferredValue 是 React 18 併發特性的實際落地方式。它們的核心思想很簡單：區分緊急更新和非緊急更新，讓瀏覽器優先處理使用者互動。不需要全面改造程式碼，在關鍵路徑上用就行。