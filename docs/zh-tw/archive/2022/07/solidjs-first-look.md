...
React 的虛擬 DOM 模式帶來了一個根本問題：每次狀態變化，整個子樹都要重新執行 render 函式，再通過 Diff 找出變化的部分。Solid 的答案是：不需要。
...
注意：`count` 是一個函式，需要 `count()` 獲取值。這是 Solid 和 React 最大的區別——沒有自動重新執行元件函式。
...
點按鈕時，`Parent` 和 `Child` 的函式體不會重新執行。Solid 直接更新 `<span>` 的文本節點。
...
function UserProfile({ user }) {
  return (
    <Show when={user()} fallback={<div>載入中...</div>}>
      <div>
        <h1>{user().name}</h1>
        <p>{user().bio}</p>
      </div>
...
Solid 的效能接近原生 JS，在所有框架中最快。

## 小結

Solid.js 值得每個 React 開發者體驗。它證明了 JSX 和響應式不一定要依賴虛擬 DOM。對於效能敏感的場景（大數據表格、即時資料流），Solid 是一個嚴肅的選擇。但生態和社群還遠不如 React，工具鏈和元件庫需要時間成熟。