...
tags:
  - 前端
readingTime: 2
description: "ES2022（ES13）已經全面落地了。TypeScript 4.7+、Node.js 18、主流瀏覽器全部支援。這篇文章整理了 ES2022 的核心特性，每個都附上實際應用場景。"
wordCount: 210
---

ES2022（ES13）已經全面落地了。TypeScript 4.7+、Node.js 18、主流瀏覽器全部支援。這篇文章整理了 ES2022 的核心特性，每個都附上實際應用場景。

## Top-Level Await

模組頂層可以直接用 await，不需要 async 函式包裹：
...
async function fetchUser(id: string) {
  try {
    ...
    throw new ApiError('獲取使用者失敗', { cause: e });
  }
}
...
try {
  ...
  console.log(e.message);              // '獲取使用者失敗'
  console.log(e.cause);                // TypeError: Failed to fetch
  console.log(e.cause.stack);          // 原始錯誤的堆疊
}
```
...
ES2022 的特性都是實用型改進：Top-Level Await 簡化模組初始化、Error.cause 完善錯誤鏈、Array.at() 終於來了。這些特性在 TypeScript 4.7+ 和 Node.js 18 中已經完全可用，沒有理由不用。