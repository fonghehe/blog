...
年初我們還在用 Webpack，年底所有新專案都跑在 Vite 上。
...
Vite 3 的釋出鞏固了它的地位。esbuild 做依賴預構建、原生 ESM 做開發模式——這個架構被證明是正確的。
...
| Turborepo | 構建編排、快取 |
| Changesets | 版本管理、釋出 |
| TypeScript Project References | 型別檢查 |
...
Vitest 的優勢太明顯了——和 Vite 共享配置、原生 ESM、速度快 3 倍。我們把 5 個專案從 Jest 遷移到 Vitest，平均耗時半天。
...
Node.js 18 內建 fetch 和測試執行器，減少了對第三方依賴的需求。Bun 的速度表現令人印象深刻，但還不適合生產環境。
...
React 18 的併發特性讓效能最佳化從「手動最佳化」變成「框架幫你最佳化」。Next.js 13 的 App Router 預示了 SSR 的未來方向。
...
// routes['/'] 的型別是 ComponentType，不是泛化的 ComponentType
```
