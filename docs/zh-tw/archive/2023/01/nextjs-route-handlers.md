...
Route Handlers 定義在 `app/api/` 目錄下的 `route.ts` 檔案中，通過匯出命名函式對應 HTTP 方法。
...
不能匯出預設函式，必須用 `GET`、`POST`、`PUT`、`DELETE`、`PATCH`、`HEAD`、`OPTIONS` 這些命名匯出。這比 Pages Router 的 `export default function handler(req, res)` 更明確。

## 動態路由與路由組

動態引數通過目錄名中的 `[param]` 獲取，與頁面路由的約定一致。
...
Route Handlers 支援返回 `ReadableStream`，適用於 SSE（Server-Sent Events）和檔案流下載。
...
export async function GET(request: NextRequest) {
  // 從 middleware 傳遞的 header 中獲取使用者資訊
  const userId = request.headers.get('x-user-id')
  const role = request.headers.get('x-user-role')

  if (role !== 'admin') {
    return NextResponse.json({ error: '需要管理員許可權' }, { status: 403 })
  }
