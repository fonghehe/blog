...
Server Actions 通過 `"use server"` 指令宣告。可以直接繫結到表單的 `action` 屬性，表單提交時自動在服務端執行。
...
export default function NewPostPage() {
  return (
    <form action={createPost} className="max-w-xl mx-auto">
      ...
      <button type="submit">釋出文章</button>
    </form>
  )
}
```

注意這裡沒有 `useState` 管理表單狀態，沒有 `onSubmit` handler，沒有 `fetch` 呼叫。整個資料流由框架處理。
...
`useFormState` 的第一個引數是 Server Action，第二個引數是初始狀態。它會將 action 的返回值作為新的 state。即使 JavaScript 載入失敗，表單也能正常提交（漸進式增強）。
...
使用者點選後 UI 立即更新，Server Action 在後臺執行。如果請求失敗，React 會自動回滾到原始狀態。這比手動管理 loading state 要簡潔得多。

## Server Actions 的實際專案建議
...
- Server Actions 用 `"use server"` 宣告，可以直接繫結表單的 `action` 屬性，省去 API 路由的編寫
- `useFormState` 處理表單狀態和錯誤反饋，支援漸進式增強
- `useOptimistic` 配合 Server Actions 實現樂觀更新，使用者體驗優於傳統 loading 方案
- 服務端必須做輸入校驗（推薦 zod），不能信任客戶端傳來的資料
- Server Actions 目前適合表單提交、簡單 CRUD 場景，複雜即時互動仍需要 API Routes