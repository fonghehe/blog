...
pnpm 8 對解析和連結階段做了最佳化，在大型 monorepo 中提升明顯：

```bash
# 我們專案的實際測試（1200+ 依賴，monorepo 含 15 個包）
# 冷安裝（clean install）
npm:   89s
yarn:  62s
pnpm7: 38s
...
pnpm 8 要求最低 Node.js 18，這是正確的決定。Node 16 已經 EME（維護期結束），放棄舊版本支援換來更好的效能。
...
```jsonc
// .npmrc
// pnpm 8 預設 strict-peer-dependencies = true
...
`workspace:*` 始終使用本地版本，`workspace:^` 釋出時會轉成 `^x.y.z`。這是 pnpm 原生支援的，不需要額外設定。
...
`public-hoist-pattern` 控製哪些依賴提升到根目錄。和 npm/yarn 不同，pnpm 隻提升你明確指定的包，避免幽靈依賴。
...
pnpm workspace + Turborepo 是目前 monorepo 最佳組合。pnpm 管依賴，Turbo 管任務編排和快取。
...
- 預設嚴格 peer dependency 處理，減少了版本衝突
- Node.js 18+ 要求，擁抱現代執行時
