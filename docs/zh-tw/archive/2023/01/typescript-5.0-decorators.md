---
title: "TypeScript 5.0 裝飾器：從 Stage 2 到 Stage 3 的蛻變"
...
最大的區別是：新裝飾器接收的是一個 `context` 物件，而不是分散的 `target`、`propertyKey`、`descriptor`。這讓裝飾器的 API 更統一、更易擴充套件。
...
新舊裝飾器不能混用。專案遷移時必須一次性全改，不能漸進式替換。`tsconfig.json` 裡需要去掉 `experimentalDecorators`。

**後設資料反射不再可用**

舊裝飾器配合 `reflect-metadata` 可以做依賴注入，新裝飾器沒有 `design:type` 等後設資料。需要後設資料的場景（比如 Angular 的 DI）暫時還是用舊方案。
...
方法裝飾器從下往上執行（像函式組合），和舊版的行為一致，但要注意引數裝飾器的執行時機。

## 小結

- TypeScript 5.0 裝飾器基於 TC39 Stage 3 標準，API 更統一
- `context` 物件替代了分散的引數，擴充套件性更好
- 引入了 `accessor` 自動訪問器概念
- 新舊裝飾器不相容，遷移需要一次性切換
- 依賴 `reflect-metadata` 的專案暫時不適合遷移
- 推薦新專案直接使用新裝飾器語法