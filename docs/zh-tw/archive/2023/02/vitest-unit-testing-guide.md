...
tags:
  ...
  - Vitest
readingTime: 2
description: "Vitest 1.0 正式釋出了。從 0.x 到 1.0，它已經從\"有趣的實驗\"變成了可以認真用在生產環境的測試框架。"
wordCount: 425
---
...
1. **ESM 支援差**：Jest 對 ESM 的支援一直半生不熟，需要各種 transform 設定
2. **配置繁瑣**：`jest.config.js` 裡一堆 `moduleNameMapper`、`transform` 規則
...
對比 Jest 設定，精簡了很多。不需要 `moduleNameMapper`，因為直接用 Vite 的 `resolve.alias`。
...
大部分專案的遷移可以在一兩天內完成。Jest 的 matcher API（`toBe`、`toEqual` 等）完全一致。
