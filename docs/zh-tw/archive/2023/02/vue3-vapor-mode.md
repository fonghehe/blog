...
tags:
  - Vue
readingTime: 3
description: "Vue 團隊在 Vue 3.3+ 版本中逐漸披露了 Vapor Mode 的設計思路。Vapor Mode 是一種編譯策略，將 Vue SFC 編譯為直接操作 DOM 的命令式程式碼，跳過虛擬 DOM 的 diff 過程。這是 Vue 對 SolidJS 和 Svelte 編譯時最佳化思路的回應。"
---

Vue 團隊在 Vue 3.3+ 版本中逐漸披露了 Vapor Mode 的設計思路。Vapor Mode 是一種編譯策略，將 Vue SFC 編譯為直接操作 DOM 的命令式程式碼，跳過虛擬 DOM 的 diff 過程。這是 Vue 對 SolidJS 和 Svelte 編譯時最佳化思路的回應。

## 虛擬 DOM 的成本

虛擬 DOM 為 Vue 提供了宣告式程式設計模型和跨平臺能力，但有固定開銷：建立虛擬節點樹、diff 演算法比較、生成更新指令。
...
Vapor Mode 下，Vue 編譯器會分析模板中每個響應式變數的影響範圍，生成直接操作 DOM 的程式碼。
...
Vapor Mode 對 `reactive()` 和 `ref()` 的使用沒有影響，響應式系統完全保留。區別在於模板更新的排程方式。
...
<template>
  <!-- Vapor Mode 會為 v-for 生成高效的列表更新演算法 -->
  <ul>
...
```ts
// Svelte：編譯時確定所有更新路徑，沒有執行時響應式系統
// 更新是編譯生成的命令式程式碼
let count = 0
// 編譯後：直接賦值 DOM，沒有響應式執行時

// SolidJS：有響應式執行時，但沒有虛擬 DOM
// 通過 createSignal + JSX 實現精確更新
const [count, setCount] = createSignal(0)
// <span>{count()}</span> 編譯為：直接繫結響應式到文本節點

// Vue Vapor Mode：保留 Vue 的響應式執行時 + 模板語法
...
<!-- 不支援的特性 -->
<script setup>
// 1. render 函式元件在 Vapor Mode 中不可用
// 需要回退到 vdom 模式
import { h } from 'vue'

const RenderFnComponent = (props) => {
  return h('div', null, props.text) // Vapor 不支援
}

// 2. <Transition>、<KeepAlive> 等依賴 vdom 的內建元件
// 需要特殊處理或回退

// 3. 動態元件 :is 在某些複雜場景下可能回退到 vdom
</script>
