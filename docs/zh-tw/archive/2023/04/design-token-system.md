---
title: "Design Token 系統：從 CSS 變數到多主題架構"
...
Design Token 是設計決策的最小單元。顏色、間距、圓角、字型——這些值不應該散落在 CSS 檔案裡，而應該集中定義、統一管理。
...
用 Style Dictionary 從 Token 原始檔生成 CSS 變數、TypeScript 型別、iOS/Android 常量。一套原始檔，多平臺輸出。

## 小結

- Design Token 是設計系統的基礎設施，讓設計決策可程式設計
- 基礎色板 + 語義化 Token 的兩層結構兼顧靈活性和可維護性
- CSS 變數方案實現零 JS 開銷的主題切換
- 配合 Style Dictionary 實現 Token 的多平臺分發
- Token 系統的建設投入大，但在多主題、多品牌專案中收益明顯