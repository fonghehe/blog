    ...
    <div>
      {posts.map((post) => (
        <article key={post.id}>
          <h2>{post.title}</h2>
          <p>{post.summary}</p>
          {/* Server Component 裡可以巢狀 Client Component */}
...
核心原則：**預設 Server Component，需要 `useState`/`useEffect`/事件處理時才加 `'use client'`**。
...
沒有 `useEffect`，沒有 loading state 管理，沒有 waterfalls。資料在服務端準備好，直接渲染。
...
- 純展示、不需要互動的元件
- 資料獲取邏輯
- 訪問後端資源（資料庫、檔案系統、內部服務）
- 敏感操作（token、金鑰不暴露給客戶端）
...
**邊界原則：** Client Component 應該儘量小、儘量在葉子節點。元件樹的大部分應該是 Server Component。

## 小結

- RSC 不是 SSR 的替代，是全新的元件模型
- 預設 Server Component，需要互動時才標記 `'use client'`
- 消除了傳統資料獲取的 loading/waterfall 問題
- 服務端邏輯不暴露給客戶端，安全性和 bundle 體積都有優勢
- 需要轉變心智模型：從"所有元件都在客戶端"到"按需選擇執行環境"