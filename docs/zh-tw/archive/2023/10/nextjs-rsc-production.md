...
tags:
  - Next.js
readingTime: 2
description: "Next.js 14 釋出了。App Router 從實驗性功能變成了推薦方案。是時候認真聊聊生產環境的實踐經驗了。"
---

Next.js 14 釋出了。App Router 從實驗性功能變成了推薦方案。是時候認真聊聊生產環境的實踐經驗了。
...
佈局在導航時不會重新渲染，保留客戶端狀態。這是 App Router 相比 Pages Router 最大的改進之一。
...
不需要手動寫 API route + fetch + state 管理。`"use server"` 標記的函式可以直接在 Server Component 中呼叫，表單提交自動走 Server Action。
...
兩個 Suspense 邊界內的元件並行載入，先返回統計數字，文章列表後到。使用者不會看到整體白屏。
...
- Server Actions 消除了傳統 API route + client fetch 的樣板程式碼
- 佈局系統在導航時保留狀態，體驗提升明顯
- 流式渲染 + Suspense 讓 TTFB 和 FCP 大幅改善
- 快取策略粒度細但概念多，需要花時間理解預設行為