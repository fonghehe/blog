---
title: "Bun 1.0 深度評測：JavaScript 執行時的新競爭者"
date: 2023-12-20 11:47:15
tags:
  - 前端
readingTime: 2
description: "Bun 1.0 釋出三個月了。在測試環境和幾個小專案中深度使用後，寫一份全面的評測。"
---

Bun 1.0 釋出三個月了。在測試環境和幾個小專案中深度使用後，寫一份全面的評測。
...
// 執行：bun test
// 速度比 Vitest 還快 2-3 倍（因為不需要 worker 執行緒開銷）
```
...
開發階段用 Bun 加速，生產環境暫時還是用 Node.js。等 Bun 的相容性和穩定性再成熟一些再切。

## 小結

- Bun 的效能優勢是真實的：包安裝、指令碼執行、HTTP 服務都明顯更快
- 內建測試執行器、打包器、包管理器，一站式體驗好
- Node.js 相容性是最大短板，但改善速度很快
- 適合新專案和開發環境，大型生產專案暫時觀望
