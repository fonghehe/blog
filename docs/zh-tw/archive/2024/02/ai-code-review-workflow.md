...
tags:
  - 前端
readingTime: 3
description: "作為前端平臺負責人，從去年開始嘗試將 AI 引入 Code Review 流程。經過幾個月的實踐，整理一下我們的經驗和踩過的坑。"
wordCount: 659
---

作為前端平臺負責人，從去年開始嘗試將 AI 引入 Code Review 流程。經過幾個月的實踐，整理一下我們的經驗和踩過的坑。
...
const SYSTEM_PROMPT = `你是一個資深前端 Code Reviewer。請審查以下程式碼變更：

重點關注：
1. 潛在的執行時錯誤
2. 效能問題（不必要的重渲染、大列表無虛擬化）
3. 安全問題（XSS、注入、敏感資訊洩露）
4. API 設計一致性
5. React/Vue 最佳實踐
...
不要檢查：程式碼格式、變數命名風格（由 Biome 處理）`;
```

### 第三層：人工 Review（AI 隻作為參考）
...
**Prompt 必須具體**。早期我們用泛泛的 prompt，AI 經常輸出 "程式碼看起來不錯" 這種廢話。明確指定檢查項後，有效建議比例從 20% 提升到 65%。

**分離關注點**。格式檢查和 lint 給 Biome，AI 專注邏輯和架構。不要讓 AI 做工具能做得更好的事。

**限製上下文**。隻發送 diff，不要發整個檔案。既省 token，又讓 AI 聚焦變更。

### 踩過的坑

**誤報率初期很高**。AI 會把合理的程式碼標記為問題。我們做了兩週的 prompt 迭代，重點是告訴 AI 專案的上下文（我們用 Vue 3 組合式 API、TypeScript strict 模式）。

**Token 消耗需控製**。大 PR 的 diff 可能很大。我們設了 500 行限製，超過的建議拆分 PR。
