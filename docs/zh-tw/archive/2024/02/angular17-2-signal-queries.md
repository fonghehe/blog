...
tags:
  - Angular
readingTime: 2
description: "Angular 17.2 於 2024 年 2 月 14 日釋出，帶來了 Signal-based Queries 的開發者預覽。繼 Angular 17.1 引入 Signal Inputs（`input()` 函式）之後，17.2 將 Signal 化範圍擴充套件到了模板查詢：`viewChild()`、`view"
---

Angular 17.2 於 2024 年 2 月 14 日釋出，帶來了 Signal-based Queries 的開發者預覽。繼 Angular 17.1 引入 Signal Inputs（`input()` 函式）之後，17.2 將 Signal 化範圍擴充套件到了模板查詢：`viewChild()`、`viewChildren()`、`contentChild()`、`contentChildren()`。

這是 Angular 團隊將整個元件 API 遷移到 Signal 體系的重要一步。
...
export class NewDemoComponent {
  ...
  // required：如果元素不存在會丟擲錯誤，返回 Signal<ElementRef>（非 undefined）
  ...
  // 但注意：在元件掛載前，viewChild() 值是 undefined
  constructor() {
    // effect 中自動追蹤
    effect(() => {
...
Angular 17.2 的 Signal Queries 是**開發者預覽**階段，API 可能在正式版中有調整。舊的 `@ViewChild`、`@ContentChild` 等裝飾器繼續工作，不會被廢棄（至少在 Angular 18 之前）。可以在新元件中逐步採用 Signal Queries，無需全量遷移。

## 總結

Angular 17.2 的 Signal Queries 讓模板查詢也融入了響應式體系，消除了 `ngAfterViewInit` 生命週期鉤子的依賴，並讓查詢結果可以直接參與 `computed()` 推導鏈。配合 Signal Inputs，Angular 元件的資料流正在變得更像純函式：輸入 → 訊號 → 模板，大幅降低心智負擔。