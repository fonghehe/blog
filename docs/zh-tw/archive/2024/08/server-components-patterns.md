...
**不要在 Server Component 中用 React hooks**。`useState`、`useEffect` 等隻在 Client Component 中可用。

**序列化邊界要注意**。Server Component 向 Client Component 傳遞 props 時，隻能傳遞可序列化的資料。Date 物件、Map、Set 都不能直接傳。

**"use client" 不是整個元件樹**。標記了 `"use client"` 的元件仍然可以在服務端渲染，隻是它的子元件會在客戶端 hydrate。
...
- 關鍵是控製好 Server/Client 邊界，最小化客戶端程式碼
