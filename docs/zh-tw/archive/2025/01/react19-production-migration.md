...
tags:
  - React
readingTime: 2
description: "React 19 正式釋出了。來整理一下從 18 遷移到 19 的實際注意事項，以及團隊踩過的坑。"
wordCount: 255
---

React 19 正式釋出了。來整理一下從 18 遷移到 19 的實際注意事項，以及團隊踩過的坑。
...
但注意：如果你的專案有很多用到 `forwardRef` 的元件庫（比如 Radix UI、shadcn/ui），這些庫會逐步更新，不需要你手動改。
...
## 服務端元件 + Server Actions 的生產實踐

```tsx
// app/products/page.tsx（服務端元件，直接訪問資料庫）
...
export async function submitForm(formData: FormData) {
  try {
    ...
    // 不要直接 throw Error，客戶端會收到 500 頁面
    // 要 return error 資訊
...
- React 19 遷移成本不高，大部分是向後相容的
- `forwardRef` 棄用，但還能用，慢慢遷移就好
- `useActionState` 替代 `useFormState`，多了 isPending
- Server Components + Server Actions 是生產級的，值得在新專案裡全面採用
