---
title: "Angular 20 正式釋出：Zoneless 穩定、Signal Forms 與 resource() API"
...
tags:
  ...
  - JavaScript
readingTime: 2
description: "Angular 20 於 2025 年 5 月正式釋出。這是 Angular 自 2022 年開始\"Signal 化轉型\"以來最重要的里程碑——Zoneless 變更檢測正式穩定，可用於生產；Signal Forms 進入開發者預覽；新的 `resource()` API 為宣告式非同步資料管理提供了原生支援。"
---

Angular 20 於 2025 年 5 月正式釋出。這是 Angular 自 2022 年開始"Signal 化轉型"以來最重要的里程碑——Zoneless 變更檢測正式穩定，可用於生產；Signal Forms 進入開發者預覽；新的 `resource()` API 為宣告式非同步資料管理提供了原生支援。

## 重大變化：zone.js 不再是預設依賴

新建 Angular 20 專案時，`zone.js` **不再預設包含在 polyfills 中**：

```bash
# 建立新的 Angular 20 專案
ng new my-app
# 選項：Would you like to use zoneless? (y/N) → 預設 y（推薦）
```

```json
// angular.json（Angular 20 新專案）
{
  "build": {
    "options": {
...
遷移老專案時，如果暫時不想移除 zone.js，可以保持原配置不變（向後相容）。
...
export class ChatComponent {
  ...
  // ✅ 第三方 WebSocket 回撥（非 Angular zone）→ 用 signal.set() 更新
  private setupWs() {
    ...
    <form (ngSubmit)="onSubmit()">
      <div class="field">
        ...
        @if (form.controls.username.hasError("minlength")(); as err) {
          <small>至少 {{ err.requiredLength }} 個字元</small>
        }
      </div>
...
注意：Signal Forms 是**開發者預覽**，API 在未來版本可能調整。舊的 `ReactiveFormsModule` 和 `FormsModule` 繼續完全支援。

## resource() API：宣告式非同步資源
