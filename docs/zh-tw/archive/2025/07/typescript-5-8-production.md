...
Node.js 22+ 支援直接執行 `.ts` 檔案（不生成 .js），但隻支援"可擦除語法"（不允許 `enum`、`namespace` 等執行時語法）。TS 5.8 新增 `--erasableSyntaxOnly` 選項來檢測違規：
...
### 問題 3：template literal 型別的遞迴限製

```typescript
// 深層巢狀的 template literal 型別會觸發 TS 的遞迴限製
...
// 實踐：限製深度或使用執行時方案（zod、yup）替代複雜的型別推導
```
