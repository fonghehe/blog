...
.tooltip {
  ...
  position-anchor: --my-anchor; /* 繫結到錨點 */
...
**推薦策略**：新專案可以用 CSS Anchor Positioning 處理簡單場景（Tooltip、下拉），複雜互動（有動畫、複雜對齊邏輯）仍可保留 Floating UI。

## 總結

CSS Anchor Positioning 是"JavaScript 做 CSS 工作的歷史終於結束了"的又一個例子。2025 年隨著 Firefox 支援到來，它已經到了"漸進增強使用"的階段。對於內部工具和 B 端系統（可以要求現代瀏覽器），現在完全可以用它替代 Popper.js/Floating UI 的基礎場景。
