...
Angular 21 將 Signal Forms 提升為穩定 API，意味著可以在生產環境放心使用：
...
Angular 21 將路由 Resolve（資料預載入）與 `resource()` 深度整合：
...
Angular 21 內建了路由級後設資料注入，無需 Angular Meta 服務：
...
# 注意：Angular 21 會對仍在使用 zone.js 的新專案顯示棄用警告
# 不影響執行，但會在構建時列印提示
```

## 總結

Angular 21 RC 是 Signal 化轉型的收尾版本——Signal Forms 穩定意味著 Angular 的整個元件 API（輸入、輸出、查詢、表單）都完成了 Signal 化。結合 Zoneless，Angular 的變更檢測從"全域性攔截"演進到"精準響應"。正式版 11 月釋出，RC 階段適合在預生產環境全面測試遷移路徑。
