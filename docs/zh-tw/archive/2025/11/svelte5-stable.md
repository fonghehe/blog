...
雙向繫結      -             v-model       $bindable()

語法簡潔度：Svelte > Vue > React
型別安全：  Svelte ≈ Vue > React
...
1. 漸進式遷移：Svelte 5 向後相容 Svelte 4 語法
2. 自動遷移工具：npx sv migrate svelte-5
3. 優先遷移：新元件用 Svelte 5 寫法
4. 後續遷移：逐步改造老元件
```
...
- $props 替代 export let，型別安全更好
- 回撥替代 createEventDispatcher，API 更簡潔
- Svelte 5 向後相容，可以漸進式遷移
- 對於小型專案和效能敏感場景，Svelte 依然是好選擇
