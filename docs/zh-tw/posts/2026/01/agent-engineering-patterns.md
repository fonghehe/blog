...
tags:
  - 前端
readingTime: 3
description: "2025 年下半年，我帶團隊把一個內部低程式碼平臺的程式碼生成模組從\"人工編寫 + AI 補全\"升級成了\"Agent 自主規劃 + 工具鏈執行\"。這個過程踩了不少坑，也總結出了一些可複用的模式。"
---

2025 年下半年，我帶團隊把一個內部低程式碼平臺的程式碼生成模組從"人工編寫 + AI 補全"升級成了"Agent 自主規劃 + 工具鏈執行"。這個過程踩了不少坑，也總結出了一些可複用的模式。
...
```
Chatbot：使用者提問 → 模型回答 → 結束
Agent：  使用者目標 → 模型規劃 → 呼叫工具 → 觀察結果 → 繼續執行 → 迴圈直到目標完成
```
...
interface AgentTask {
  goal: string;           // 使用者目標描述
  context: ProjectContext; // 專案上下文（檔案樹、依賴、已有程式碼）
  tools: ToolRegistry;     // 可用工具集
  maxSteps: number;        // 最大執行步數
  onStep: (step: AgentStep) => void; // 每步回撥
}

interface AgentStep {
  thought: string;      // Agent 的推理過程
  action: ToolCall;     // 選擇的工具和引數
...
最基礎的 Agent 模式是 ReAct（Reasoning + Acting）。前端開發中最常見的應用是**檔案操作 Agent**：
...
實際使用時，工具集可以包括：讀檔案、寫檔案、執行命令、搜尋程式碼等：
...
class OrchestratorAgent {
  async decompose(goal: string): Promise<SubTask[]> {
    const response = await this.llm.complete(`
      ...
      每個子任務需要指定：
      1. 負責的 Agent 型別（coder / reviewer / tester）
      2. 具體描述
      3. 依賴的前置任務 ID
    `);
...
Agent 不應該完全自主。我們在關鍵決策點設定**人機協作節點**：
...
interface HumanApprovalNode {
  type: 'approval';
  prompt: string;
  options: string[];        // 使用者可選的操作
  defaultAction?: string;   // 超時後的預設操作
  timeoutMs?: number;
}
