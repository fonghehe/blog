...
不同的程式碼變更需要不同的審查策略。小改動用自動審查，大重構用深度分析模式，安全敏感的變更必須人工+AI 聯合審查。

```yaml
# .code-review.yaml —— 團隊的 AI 審查設定
review:
  modes:
    auto:
      # < 50 行變更，自動審查，自動批準
...
export const reviewPrompts = {
  performance: `
審查以下程式碼變更的效能問題，重點關注：
1. 不必要的重渲染：檢查 React 元件是否有缺失的 memo/useMemo/useCallback
2. 資料獲取：檢查是否有 N+1 查詢、缺失的快取策略
3. Bundle 影響：新增的依賴是否可以用已有依賴替代
4. 記憶體洩漏：檢查未清理的 subscription、timer、observer

只報告真正有問題的地方，不要提"程式碼風格"或"可以加註釋"。
如果有問題，給出具體的修復程式碼。
  `,
...
1. 匯出的 API 是否有簽名變化
2. 元件 props 是否有刪除或型別變更
3. CSS class 名是否變化（對外部消費者有影響）
4. 環境變數或配置項是否變更

如果檢測到 breaking change，說明影響範圍並建議遷移方案。
  `,

  security: `
審查此變更的安全性：
1. 使用者輸入是否經過校驗和轉義
2. 敏感資料是否被正確處理（不記日誌、不暴露到前端）
3. 權限校驗是否到位
...
## 將 AI 審查整合到 CI/CD 流水線

AI 審查不是替人做決策，而是幫人做資訊篩選。在 CI 中整合時，關鍵是讓 AI 做初篩，把需要人工關注的點高亮出來。
...
最大的陷阱是"AI 說沒問題就真的沒問題"。AI 會遺漏業務邏輯錯誤、上下文依賴的問題、以及團隊約定俗成的隱含規則。我的做法是：AI 負責機械性檢查（型別安全、效能模式、常見 bug pattern），人負責業務正確性和架構一致性。
...
- CI 整合時 AI 做初篩，人工做終審，不要反過來
- AI 審查最大的價值是"減少人看垃圾資訊的時間"，而不是替代人的判斷
