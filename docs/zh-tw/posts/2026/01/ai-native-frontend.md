...
2024 年我們還在用 Copilot 補全程式碼，2025 年 Agentic Coding 開始普及，到了 2026 年，主流工作流已經是"人類定目標，AI 全流程執行"。核心區別在於：AI 不再只是建議程式碼片段，而是理解整個專案上下文、讀寫檔案、執行測試、迭代修復。
...
export const frontendAgent = defineAgent({
  ...
  rules: [
    '所有新元件必須使用 React Server Components',
...
實際專案中，AI 程式碼生成已經遠超"生成函式"這個層面。現在的 Agent 可以一次性生成整個功能模組——包括元件、hooks、測試、路由配置、甚至對應的 API mock。

```tsx
// 自然語言輸入：
// "建立一個支援無限滾動的商品列表，使用 TanStack Query v5 做資料獲取，
//  IntersectionObserver 做滾動載入，骨架屏做 loading 狀態"
...
Bun 3.x 內建了 AI 外掛介面，Vite 7 也支援 AI-driven HMR 最佳化。構建工具不再只是打包程式碼，而是主動分析依賴圖譜、預測性預載入、甚至自動拆包。

```typescript
// vite.config.ts —— AI 驅動的智慧拆包配置
...
export default defineConfig({
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        manualChunks: undefined, // 交給 AI 外掛處理
      },
    },
  },
});
...
AI 不是萬能的。我堅持的原則：認證/授權邏輯必須人工審查，敏感資料處理禁止 AI 生成，生產環境的基礎設施配置必須人工 review。AI 生成的程式碼在合入主分支前必須通過靜態分析和安全掃描。
...
- AI Native 開發的核心轉變是從"程式碼補全"到"Agent 全流程協作"
- 自然語言驅動的功能開發已經成熟，但安全邊界必須明確
- 構建工具鏈正在深度整合 AI，智慧拆包和預測性預載入是趨勢
- 人工審查在安全敏感領域仍然不可替代
