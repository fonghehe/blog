...
tags:
  - React
readingTime: 3
description: "Next.js 17 在 2025 年底釋出，帶來了幾個底層架構的重大變化。最核心的轉變是 Turbopack 正式取代 Webpack 成為預設構建器、以及新的 PPR（Partial Prerendering）穩定版。對於大型專案來說，這些變化的影響是實質性的。"
---

Next.js 17 在 2025 年底釋出，帶來了幾個底層架構的重大變化。最核心的轉變是 Turbopack 正式取代 Webpack 成為預設構建器、以及新的 PPR（Partial Prerendering）穩定版。對於大型專案來說，這些變化的影響是實質性的。

## Turbopack：終於告別 Webpack

Turbopack 經歷了 3 年的開發和打磨，終於在 Next.js 17 中成為預設構建器。對於我們 200+ 頁面的專案，冷啟動時間從 Webpack 的 45 秒降到了 3 秒以內。
...
Next.js 17 的 Cache API 比以前的 `fetch` 快取策略更加精細和可控。你可以精確控制每個資料的快取時間、失效策略、和預熱邏輯。
...
中介軟體在 Next.js 17 中獲得了更強的能力，包括直接訪問資料庫連線、AI 推理呼叫、以及更靈活的響應構造。
...
- Turbopack 正式取代 Webpack，大型專案的構建速度提升是數量級的
- PPR 結合了 SSG 的速度和 SSR 的動態性，是 Next.js 最具創新性的特性
- 新的 Cache API 讓快取控制從"全有或全無"變成精確到資料級別的控制
- 中介軟體能力大幅增強，鑑權、A/B 測試、速率限制可以在邊緣完成
