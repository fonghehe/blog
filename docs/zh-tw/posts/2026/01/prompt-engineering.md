...
date: 2026-01-21 14:55:32
tags:
  - 前端
readingTime: 3
description: "Prompt Engineering 在 2026 年仍然是前端工程師需要掌握的核心技能。雖然模型越來越強，但\"會提問\"和\"不會提問\"之間的差距反而更大了。好的 prompt 能讓 AI 一次產出可用程式碼，差的 prompt 讓你在對話中浪費半小時。"
wordCount: 417
---

Prompt Engineering 在 2026 年仍然是前端工程師需要掌握的核心技能。雖然模型越來越強，但"會提問"和"不會提問"之間的差距反而更大了。好的 prompt 能讓 AI 一次產出可用程式碼，差的 prompt 讓你在對話中浪費半小時。
...
元件模式：Server Component 優先，需要客戶端互動時使用 'use client'
狀態管理：本地狀態用 useState/useReducer，全域性狀態用 Zustand
錯誤處理：所有非同步操作必須有 error boundary 兜底

要求：
- 元件名：${spec.name}
- 功能：${spec.description}
- Props 介面：${JSON.stringify(spec.props, null, 2)}
- 必須覆蓋的狀態：loading, error, empty, success
- 必須支援的無障礙：keyboard navigation, ARIA labels, focus management
...
分析維度：
1. 渲染效能：不必要的重渲染、缺失的 memoization
2. 包體積：可替代的重型依賴、未使用的匯入
3. 執行時效率：迴圈中的物件建立、閉包陷阱
4. 網路效能：瀑布式請求、缺失的預載入

輸出格式：
- 每個問題標註嚴重程度（critical/high/medium/low）
- 給出最佳化前後的程式碼對比
- 預估最佳化效果（減少的渲染次數、減少的 bundle size）
...
通用 prompt 只能產出通用程式碼。讓 AI 理解你的專案上下文是產出可用程式碼的關鍵。我的做法是維護一個 project-context.md 檔案，每次對話時注入。

```markdown
<!-- project-context.md —— 注入到每次 AI 對話的專案上下文 -->
...
## 編碼約定
- 元件用 PascalCase，檔案用 kebab-case
- Hook 以 use 開頭，放在 hooks/ 目錄
- API 呼叫統一封裝在 api/ 目錄，使用自定義 fetch wrapper
- 型別定義優先放在 types/ 目錄，元件內型別不超過 3 行
...
```typescript
// context-loader.ts —— 自動注入專案上下文的工具
...
const cotSteps = [
  {
    step: 1,
    prompt: `
先不寫程式碼，分析這個功能需要哪些資料結構和介面。
列出所有 TypeScript 型別定義。
    `,
    output: 'types/',
  },
  {
    ...
    prompt: `
基於上面的型別定義，實現資料獲取層。
包括 API 呼叫函式和 TanStack Query hooks。
    `,
    output: 'api/ + hooks/',
  },
  {
    step: 3,
    prompt: `
基於上面的 hooks，實現 UI 元件。
從最底層的原子元件開始，逐步構建到頁面級元件。
    `,
    output: 'components/',
  },
  {
...
現在，按照同樣的程式碼風格實現 useThrottle hook。
`;
```
