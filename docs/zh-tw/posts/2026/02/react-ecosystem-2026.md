...
Redux Toolkit 仍然維護，但新專案幾乎沒有選它的了。Zustand 5.x 以其極致簡潔的 API 和對 React Compiler 的完美支援，成為事實標準。
...
## 資料獲取：TanStack Query 穩坐第一

TanStack Query v5 已經是資料獲取的絕對標準。SWR 仍在維護但更新緩慢，Apollo Client 退守 GraphQL 特定場景。
...
CSS-in-JS 在 2026 年基本退出了主流。Tailwind CSS v4 的 JIT 引擎效能大幅提升，加上零執行時的優勢，成為了預設選擇。

```tsx
// 2026 年的樣式組合策略
// Tailwind 用於佈局和通用樣式
// CSS Modules 用於元件特有的複雜樣式
...
## UI 元件庫：shadcn/ui 的生態統治

shadcn/ui 不是一個傳統意義上的元件庫——它是一套可複製的元件程式碼。2026 年，這個模式被證明是正確的：你需要的不是 npm 包，而是你擁有的、可自由修改的元件程式碼。
...
// 元件安裝後直接在 src/components/ui/ 中，可以自由修改
// 這是 shadcn/ui 最大的優勢：你擁有程式碼

// 自定義擴充套件示例：在 shadcn 的 Dialog 基礎上封裝業務元件
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const toolchain2026 = {
  ...
  deprecated: [
    ...
    'Styled-components / Emotion —— 執行時 CSS-in-JS 退出主流',
...
- 狀態管理選 Zustand，資料獲取選 TanStack Query，這是 2026 年無爭議的預設組合
- CSS-in-JS 已死，Tailwind v4 + CSS Modules 是當前最務實的樣式方案
- shadcn/ui 的"擁有程式碼"模式是元件庫的未來方向
