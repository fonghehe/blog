---
title: "CSS View Transitions Level 2：跨檔案動畫實戰"
...
CSS View Transitions Level 1 讓單頁應用（SPA）的頁面切換動畫變得優雅。而 2026 年正式進入各大瀏覽器穩定通道的 **View Transitions Level 2**，將這一能力擴充套件到了多頁應用（MPA）——無需 JavaScript 框架，純 CSS + HTML 即可實現流暢的跨檔案過渡動畫。

## Level 1 回顧：SPA 中的檢視過渡
...
Level 2 的核心突破是讓普通多頁網站也能實現無縫導航動畫，只需在 CSS 中宣告：

```css
/* 在 <head> 中的全域性樣式裡新增 */
@view-transition {
  navigation: auto; /* 告訴瀏覽器對普通導航啟用檢視過渡 */
}
```
...
Level 2 最強大的功能是跨文件的"英雄動畫"（hero animation）——同一元素在不同頁面間的連續過渡：
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="zh">
      <head>
        {/* Level 2 的 MPA 模式通過 CSS 啟用，無需額外 JS */}
      </head>
      <body>{children}</body>
    </html>
  );
...
CSS View Transitions Level 2 填補了多頁應用動畫的最後一塊拼圖。不再需要 JavaScript 框架介入，不再需要手動管理動畫狀態，只需幾行 CSS 宣告，你的網站就能擁有媲美原生應用的頁面過渡體驗。對於 2026 年新建的 MPA 專案，這應該是預設啟用的標配功能。
