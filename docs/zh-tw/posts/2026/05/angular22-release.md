---
title: "Angular 22 正式釋出：Evergreen 編譯器驅動的新時代"
...
RC 階段大量社群專案參與了測試，正式版彙總了真實資料：

```
社群反饋統計（來自 Angular GitHub Discussions）：
...
Angular 22 對服務端渲染架構做了徹底重構，正式引入流式 SSR 和邊緣渲染支援：
...
export const serverConfig: ApplicationConfig = {
  providers: [
    provideServerRendering(
      // 22 新增：流式 SSR，首位元組時間大幅提前
      withStreamingSSR({
        // shell 內容立即傳送，資料載入完成後流式追加
        shellTimeout: 100, // ms
      }),
    ),
  ],
...
| ---------- | -------------- | ----------------- | -------------------- |
| 響應式模型 | Signal（內建） | Signal（內建）    | Vapor（Signal-like） |
| 編譯期最佳化 | Evergreen ⭐   | React Compiler    | Vapor 編譯器         |
| SSR 支援   | 流式 + 邊緣    | Server Components | Nuxt 5 整合          |
| 型別安全   | 全鏈路 TS      | 全鏈路 TS         | 全鏈路 TS            |
| 學習曲線   | 中高           | 中                | 低中                 |
| 企業採用   | 高             | 高                | 中                   |
...
2. **Signal Router 遷移**：現有 Routes 陣列配置仍然有效，無需立即遷移；但新路由功能僅在 Signal Router 中可用
3. **`NgModule` 逐步移除**：建議使用 `ng generate @angular/core:remove-unused-ngmodules` 清理空的 NgModule

## 總結

Angular 22 標誌著 Angular 歷時三年的現代化轉型全面完成。從 Angular 14 引入獨立元件、16 帶來 Signal 預覽、20 完成 Zoneless 穩定，到如今 22 的 Evergreen 編譯器與 Signal Router，這條路線一以貫之。對於新專案，Angular 22 提供的開發體驗已經足夠流暢；對於存量專案，平滑的遷移路徑讓升級不再是一場豪賭。
