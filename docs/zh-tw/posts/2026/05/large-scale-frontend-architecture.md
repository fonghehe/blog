...
tags:
  ...
  - 前端工程化
readingTime: 6
description: "當一個前端應用的程式碼量超過 100 萬行、參與開發的團隊超過 5 個時，單體應用的架構會在多個維度同時崩塌：構建時間指數增長、團隊間程式碼衝突頻繁、釋出互相阻塞。本文討論大規模前端系統在架構層面的真實演進路徑，包括微前端的取捨決策、跨團隊協作的工程約束，以及 2026 年構建工具鏈的選型邏輯。"
wordCount: 929
---

當一個前端應用的程式碼量超過 100 萬行、參與開發的團隊超過 5 個時，單體應用的架構會在多個維度同時崩塌：構建時間指數增長、團隊間程式碼衝突頻繁、釋出互相阻塞。本文討論大規模前端系統在架構層面的真實演進路徑，包括微前端的取捨決策、跨團隊協作的工程約束，以及 2026 年構建工具鏈的選型邏輯。

## 微前端：不是銀彈，是權衡

### 為什麼需要微前端

微前端的核心動機不是"技術架構的優雅"，而是**組織架構的對映**。康威定律在前端同樣成立：

```
組織結構：
┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
│  訂單團隊  │  │  商品團隊  │  │  使用者團隊  │  │  營銷團隊  │
└──────────┘  └──────────┘  └──────────┘  └──────────┘

產品形態（單體時期）：
┌─────────────────────────────────────────────────────┐
│                   一個巨型 SPA                        │
│  訂單模組 ←→ 商品模組 ←→ 使用者模組 ←→ 營銷模組         │
└─────────────────────────────────────────────────────┘
問題：4 個團隊共享一個倉庫、一條 CI、一個釋出節奏

產品形態（微前端）：
┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐
│  訂單 MF  │  │  商品 MF  │  │  使用者 MF  │  │  營銷 MF  │
└─────────┘  └─────────┘  └─────────┘  └─────────┘
         └───────── Shell（路由 + 全域性狀態）─────────┘
結果：獨立開發、獨立部署、獨立回滾
```
...
| Web Components                   | ✅ Shadow DOM | ❌ 各自打包    | attribute / event   | 輕量元件級整合   |

### Module Federation 2.0 的工程實踐

2026 年 Module Federation 已經成為同技術棧微前端的首選方案，Vite 通過 `@module-federation/vite` 原生支援：
...
// Remote 應用通過 shared 獲取同一個 pinia 例項
// 在 remote 中直接 import { useUserStore } from 'shell/shared-state'
```

## 跨團隊協作的工程約束

### 介面契約：Schema-First 開發

當多個團隊開發獨立部署的微前端時，元件間通訊必須有明確的介面契約：
...
shared-ui/
├── packages/
│   ├── primitives/     # 原子元件（Button, Input, Modal）
│   ├── composites/     # 組合元件（SearchBar, DataTable）
...
- `composites`：允許更頻繁的迭代，通過 feature flag 漸進式釋出
...
│  Rollup (Vite 預設) / Rolldown (Vite 6+)    │
├─────────────────────────────────────────────┤
│  底層編譯層 (Transform)                       │
...
Vite 6 引入 Rolldown（Rust 實現的 Rollup 相容打包器）作為可選構建引擎：
...
| 場景            | 推薦方案                     | 原因                                    |
| --------------- | ---------------------------- | --------------------------------------- |
| 業務應用        | Vite 6 (Rolldown)            | 開發體驗 + 生產效能的最佳平衡           |
| 元件庫          | Vite Library Mode / tsup     | 多格式輸出（ESM + CJS），treeshake 友好 |
| CLI 工具        | tsup (ESBuild)               | 純轉譯，無需複雜打包                    |
| 超大型 Monorepo | Turbopack / Vite + Turborepo | 增量構建 + 遠端快取                     |
...
const BUDGET_RULES: BudgetRule[] = [
  ...
  { pattern: "vue-vendor-*.js", maxSize: 50 * 1024 }, // Vue 執行時 < 50KB
