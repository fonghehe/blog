...
tags:
  ...
  - 架構
readingTime: 6
description: "Signal 已經成為 Angular 新架構的關鍵能力。本文討論如何用 Signal 建模局部狀態、衍生狀態和跨模組資料流，並給出大型應用中的治理建議。"
wordCount: 1308
---

Angular 的 Signal 體系已經從新特性變成架構設計的一部分。它改變的不隻是狀態更新語法，更重要的是讓元件、服務和範本之間的資料依賴變得更清晰。對於大型應用來說，Signal 的價值在於減少隱式訂閱，讓狀態變化更可追蹤。

## 從局部狀態開始

不是所有狀態都需要進入全域 store。2026 年的最佳實踐是按狀態的生命週期和作用域來選擇容器：

**元件內 Signal（Component Signals）**
適用於表單輸入、彈窗開關、當前 tab、局部篩選條件等。它們生命週期短、依賴範圍明確，放在元件內部反而更容易維護：
...
判斷標準很明確：**當一個狀態隻被當前元件及其直接子元件使用時，它就應該留在元件裡。**

## 服務層 Signal：領域狀態的管理

當狀態需要跨越多個元件時，提升到服務層。但要注意服務層 Signal 的暴露方式：
...
這個模式的關鍵設計決策：
- **私有 Signal + 公開 computed**：外部隻能讀取衍生資料
- **命令方法作為修改入口**：所有修改都有明確的呼叫來源，方便追蹤和除錯
...
**computed 應該用於描述「資料是什麼」，而不是「在資料變化時要做什麼」。**
...
副作用（網路請求、埋點、持久化、DOM 操作）應該放在 `effect()` 或顯式的服務方法裡：
...
Angular 在 2026 年引入了 `linkedSignal`，用於解決「當源 Signal 變化時自動重置衍生 Signal」的常見需求。一個典型場景：
...
| WebSocket 即時資料 | RxJS → Signal | RxJS 處理流，`toSignal()` 轉換後給元件消費 |
| 多源資料聚合 | RxJS | `combineLatest` 依賴追蹤更靈活 |
...
推薦的架構模式：**服務層用 RxJS 處理複雜非同步編排，元件層用 Signal 消費——`toSignal()` 作為兩者之間的橋。** 這不是什麼哲學偏好，而是純粹的工程合理性：每個層使用最適合的工具。

## 大型應用中的 Signal 治理

當專案變得龐大後，沒有規則約束的 Signal 會變成新的混亂來源。幾個實用的治理原則：

**原則 1：明確所有權**
- 元件擁有 UI 狀態 Signal（表單值、展開/摺疊、當前選中項）
- 領域服務擁有業務狀態 Signal（購物車、使用者權限、當前訂單）
- 基礎設施服務不擁有狀態，隻暴露方法和 Observable

**原則 2：控製寫入入口**
不要讓任何元件都能直接修改共享 Signal。遵循 CQRS 思想：
- 讀取通過 `computed` 或唯讀 Signal 暴露
...
**原則 4：避免 Signal 地獄**
不要因為 Signal 好用就把所有資料都變成 Signal。靜態設定資料（如國家列表、省市區資料）用普通常數更好。隻會在一個地方用到的衍生資料，直接用函式計算可能比 computed 更簡單。

## 小結

Angular Signal 架構的關鍵不是把 RxJS 全部替換掉，而是把不同型別的響應式問題放到合適的位置。Signal 負責清晰的同步狀態和簡單的衍生關係，RxJS 繼續處理複雜非同步流和即時資料編排。`linkedSignal` 讓「源-目標重置」模式變得更安全。對於大型應用來說，Signal 治理的核心是：明確所有權、控製寫入入口、保持衍生純粹——這些原則和大規模軟體工程的基本原則完全一致。2026 年的 Angular 開發者需要同時掌握 Signal 和 RxJS，知道在什麼場景下用哪個，才是真正的工程成熟度。
